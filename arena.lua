...
function Arena:on_enter(from, level, loop, units, passives, shop_level, shop_xp, lock)
  ...
  if self.level == 1000 then
    self.level_1000_text = Text2({
      group = self.ui,
      x = gw / 2,
      y = gh / 2,
      lines = { { text = "[fg, wavy_mid]SNKRX", font = fat_font, alignment = "center" } },
    })
  elseif (self.level - (25 * self.loop)) % 6 == 0 or self.level % 25 == 0 then
    ...
    self.t:after(1, function()
        ...
        function()
          ...
          self.t:every(function()
            if self.boss and not self.boss.dead then
              return (#self.main:get_objects_by_classes(self.enemies) <= 1)
                and not self.spawning_enemies
            elseif self.boss and self.boss.dead then
              return (#self.main:get_objects_by_classes(self.enemies) <= 0)
                and not self.spawning_enemies
            end
          end, function()
            self.hfx:use("condition1", 0.25, 200, 10)
            ...
            self.t:after(0.5, function()
              ...
              self.t:after((8 + math.floor(self.level / 2)) * 0.1 + 0.5 + 0.75, function()
                self.spawning_enemies = false
              end, "spawning_enemies")
              local spawn_type = random:table({ "left", "middle", "right" })
              local spawn_points = {
                left = { x = self.x1 + 32, y = gh / 2 },
                middle = { x = gw / 2, y = gh / 2 },
                right = { x = self.x2 - 32, y = gh / 2 },
              }
              local p = spawn_points[spawn_type]
              SpawnMarker({ group = self.effects, x = p.x, y = p.y })
              self.t:after(1.125, function()
                self:spawn_n_enemies(p, nil, 8 + math.floor(self.level / 2))
              end)
            end)
          end)
        end
      ...
      self.t:every(function()
        return self.start_time <= 0
          and (self.boss and self.boss.dead)
          and #self.main:get_objects_by_classes(self.enemies) <= 0
          and not self.spawning_enemies
      ...
      self.t:every(
        ...
        function()
          ...
          self.t:every(function()
            ...
            if self.wave > self.max_waves then
              return
            end
            self.hfx:use("condition1", 0.25, 200, 10)
            self.hfx:pull("condition2", 0.0625)
            self.t:after(0.5, function()
              if random:bool(self.level_to_distributed_enemies_chance[self.level]) then
                ...
                SpawnMarker({ group = self.effects, x = p.x, y = p.y })
                self.t:after(1.125, function()
                  self:spawn_n_enemies(
                    p,
                    nil,
                    8 + (self.wave + math.min(self.loop * 12, 200) - 1) * 2
                  )
                end)
              end
            end)
    ...
    if self.level == 20 and self.trailer then
      Text2({
        ...
        x = gw / 2,
        y = gh / 2 - 24,
        lines = { { text = "[fg, wavy]SNKRX", font = fat_font, alignment = "center" } },
      })
      Text2({
        group = self.ui,
        x = gw / 2,
        ...
        sx = 0.5,
        sy = 0.5,
        lines = { { text = "[fg, wavy_mid]play now!", font = fat_font, alignment = "center" } },
      })
      Text2({
        group = self.ui,
        x = gw / 2,
        ...
        lines = {
          {
            text = "[light_bg, wavy_mid]music: kubbi - ember",
            font = fat_font,
            alignment = "center",
          },
        },
  ...
  if self.level == 1 then
    local t1 = Text2({
      ...
      sy = 0.6,
      lines = {
        { text = "[light_bg]<- or a         -> or d", font = fat_font, alignment = "center" },
      },
    })
    local t2 = Text2({
      group = self.floor,
      ...
      lines = {
        {
          text = "[light_bg]turn left                                      turn right",
          font = pixul_font,
          alignment = "center",
        },
      },
    ...
    local t3 = Text2({
      ...
      sx = 0.6,
      sy = 0.6,
      lines = { { text = "[light_bg]esc - options", font = fat_font, alignment = "center" } },
    })
    t1.t:after(8, function()
      t1.t:tween(0.2, t1, { sy = 0 }, math.linear, function()
        t1.sy = 0
      end)
    end)
    t2.t:after(8, function()
      t2.t:tween(0.2, t2, { sy = 0 }, math.linear, function()
        t2.sy = 0
      end)
    end)
    t3.t:after(8, function()
      t3.t:tween(0.2, t3, { sy = 0 }, math.linear, function()
        t3.sy = 0
      end)
    end)
  end
  ...
  for _, f in ipairs(self.player.followers) do
    table.insert(units, f)
  end
  ...
  self.t:every(8, function()
    ...
    if self.arena_clear_text then
      return
    end
    if self.quitting then
      return
    end
    if self.spawning_enemies then
      return
    end
    if self.won then
      return
    end
    if self.choosing_passives then
      return
    end

    local n = self.enemy_spawns_prevented
    if math.floor(n / 4) <= 0 then
      return
    end
    self.spawning_enemies = true
    ...
    self.t:after({ 0, 0.2 }, function()
      ...
      SpawnMarker({ group = self.effects, x = p.x, y = p.y })
      self.t:after(1.125, function()
        self:spawn_n_enemies(p, 1, math.floor(n / 4), true)
      end)
    end)
    self.t:after({ 0, 0.2 }, function()
      local p = random:table_remove(spawn_points)
      SpawnMarker({ group = self.effects, x = p.x, y = p.y })
      self.t:after(1.125, function()
        self:spawn_n_enemies(p, 2, math.floor(n / 4), true)
      end)
    end)
    self.t:after({ 0, 0.2 }, function()
      local p = random:table_remove(spawn_points)
      SpawnMarker({ group = self.effects, x = p.x, y = p.y })
      self.t:after(1.125, function()
        self:spawn_n_enemies(p, 3, math.floor(n / 4), true)
      end)
    end)
    self.t:after({ 0, 0.2 }, function()
      local p = random:table_remove(spawn_points)
      SpawnMarker({ group = self.effects, x = p.x, y = p.y })
      self.t:after(1.125, function()
        self:spawn_n_enemies(p, 4, math.floor(n / 4), true)
      end)
    end)
    self.t:after(1.125 + math.floor(n / 4) * 0.25, function()
      self.spawning_enemies = false
    end, "spawning_enemies")
    self.enemy_spawns_prevented = 0
  end)
...
function Arena:update(dt)
  ...
  if
    ...
    and not self.in_credits
    and not self.choosing_passives
  then
  ...
  if self.level % 25 == 0 then
    ...
    if not self.win_text and not self.win_text2 then
      ...
      self.win_text = Text2({
        ...
        force_update = true,
        lines = {
          { text = "[wavy_mid, cbyc2]congratulations!", font = fat_font, alignment = "center" },
        },
      })
      trigger:after(2.5, function()
        ...
        self.build_text = Text2({
          ...
          y = 20,
          force_update = true,
          lines = { { text = "[wavy_mid, fg]your build", font = pixul_font, alignment = "center" } },
        })
        for i, unit in ipairs(self.units) do
          ...
          Text2({
            group = self.ui,
            x = 20 + 14 + pixul_font:get_text_width(unit.character) / 2,
            y = 40 + (i - 1) * 19,
            force_update = true,
            lines = {
              {
                text = "[" .. character_color_strings[unit.character] .. "]" .. unit.character,
                font = pixul_font,
                alignment = "left",
              },
            },
        ...
        if current_new_game_plus == 6 then
          ...
          self.win_text2 = Text2({
            ...
            lines = {
              {
                text = "[fg]now you've really beaten the game!",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 1.24,
              },
              {
                text = "[fg]thanks a lot for playing it and completing it entirely!",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 1.24,
              },
              {
                text = "[fg]this game was inspired by:",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 3.5,
              },
              {
                text = "[fg]so check them out! and to get more games like this:",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 3.5,
              },
              {
                text = "[wavy_mid, yellow]thanks for playing!",
                font = pixul_font,
                alignment = "center",
              },
            },
          ...
          Button({
            ...
            y = gh / 2 + 12,
            force_update = true,
            button_text = "nimble quest",
            fg_color = "bluem5",
            bg_color = "blue",
            action = function(b)
              open_url(b, "https://store.steampowered.com/app/259780/Nimble_Quest/")
            end,
          })
          Button({
            group = self.ui,
            ...
            y = gh / 2 + 12,
            force_update = true,
            button_text = "dota underlords",
            fg_color = "bluem5",
            bg_color = "blue",
            action = function(b)
              open_url(b, "https://store.steampowered.com/app/1046930/Dota_Underlords/")
            end,
          })
        else
          self.win_text2 = Text2({
            ...
            lines = {
              ...
              {
                text = "[fg]if you liked it:",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 3.5,
              },
              {
                text = "[fg]and if you liked the music:",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 3.5,
              },
    ...
    if not self.arena_clear_text then
      self.arena_clear_text = Text2({
        ...
        x = gw / 2,
        y = gh / 2 - 48,
        lines = { { text = "[wavy_mid, cbyc]arena clear!", font = fat_font, alignment = "center" } },
      })
    end
    self:gain_gold()
...
function Arena:set_passives(from_reroll)
  ...
  if passive_1 then
    table.insert(
      ...
      PassiveCard({
        group = main.current.ui,
        x = gw / 2 - w / 2 + 0 * (card_w + 20) + card_w / 2 + 45,
        y = gh / 2 - 20,
        w = card_w,
        h = card_h,
        card_i = 1,
  ...
  if passive_2 then
    table.insert(
      ...
      PassiveCard({
        group = main.current.ui,
        x = gw / 2 - w / 2 + 1 * (card_w + 20) + card_w / 2 + 45,
        y = gh / 2,
        w = card_w,
        h = card_h,
        card_i = 2,
  ...
  if passive_3 then
    table.insert(
      ...
      PassiveCard({
        group = main.current.ui,
        x = gw / 2 - w / 2 + 2 * (card_w + 20) + card_w / 2 + 45,
        y = gh / 2 - 20,
        w = card_w,
        h = card_h,
        card_i = 3,
  ...
  if passive_4 then
    table.insert(
      ...
      PassiveCard({
        group = main.current.ui,
        x = gw / 2 - w / 2 + 3 * (card_w + 20) + card_w / 2 + 45,
        y = gh / 2,
        w = card_w,
        h = card_h,
        card_i = 4,
  ...
  self.passive_text = Text2({
    group = self.ui,
    x = gw / 2 + 45,
    y = gh / 2 - 75,
    lines = { { text = "[fg, wavy]choose one", font = fat_font, alignment = "center" } },
  })
  ...
  if not self.died_text and not self.won and not self.arena_clear_text then
    ...
    self.t:after(2, function()
      ...
      self.death_info_text = Text2({
        ...
        lines = {
          {
            text = "[wavy_mid, fg]level reached: [wavy_mid, yellow]" .. self.level,
            font = fat_font,
            alignment = "center",
          },
        },
...
function Arena:create_credits()
  ...
  Text2({
    ...
    x = 60,
    y = 20,
    lines = { { text = "[bg10]main dev: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 117,
  ...
  Text2({
    ...
    x = 60,
    y = 50,
    lines = { { text = "[bg10]mobile: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 144,
  ...
  Text2({
    ...
    x = 60,
    y = 80,
    lines = { { text = "[blue]libraries: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 113,
  ...
  Text2({
    ...
    x = 60,
    y = 110,
    lines = { { text = "[green]music: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 100,
  ...
  Text2({
    ...
    x = 60,
    y = 140,
    lines = { { text = "[yellow]sounds: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 135,
  ...
  Button({
    ...
    x = 279,
    y = 140,
    button_text = "jcallison",
    fg_color = "yellowm5",
    bg_color = "yellow",
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/jcallison/sounds/258269/")
    end,
  })
  Button({
    group = self.credits,
    x = 342,
    y = 140,
    button_text = "hybrid_v",
    fg_color = "yellowm5",
    bg_color = "yellow",
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/Hybrid_V/sounds/321215/")
    end,
  })
  Button({
    group = self.credits,
    x = 427,
    y = 140,
    button_text = "womb_affliction",
    fg_color = "yellowm5",
    bg_color = "yellow",
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/womb_affliction/sounds/376532/")
    end,
  })
  Button({
    group = self.credits,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/bajko/sounds/399656/")
    end,
  })
  Button({
    group = self.credits,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/benzix2/sounds/467951/")
    end,
  })
  Button({
    group = self.credits,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://store.steampowered.com/developer/T_TGames")
    end,
  })
  Button({
    group = self.credits,
    x = 262,
    y = 160,
    button_text = "InspectorJ",
    fg_color = "yellowm5",
    bg_color = "yellow",
    credits_button = true,
    action = function(b)
      open_url(b, "https://freesound.org/people/InspectorJ/sounds/458586/")
    end,
  })
  Text2({
    group = self.credits,
    x = 70,
    y = 190,
    lines = { { text = "[red]playtesters: ", font = pixul_font } },
  })
  Button({
    group = self.credits,
    x = 130,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/JofersGames")
    end,
  })
  Button({
    group = self.credits,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/ekunenuke")
    end,
  })
  Button({
    group = self.credits,
    x = 224,
    y = 190,
    button_text = "cvisy_GN",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/cvisy_GN")
    end,
  })
  Button({
    group = self.credits,
    x = 292,
    y = 190,
    button_text = "Blue Fairy",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/blue9fairy")
    end,
  })
  Button({
    group = self.credits,
    x = 362,
    y = 190,
    button_text = "Phil Blank",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/PhilBlankGames")
    end,
  })
  Button({
    group = self.credits,
    x = 440,
    y = 190,
    button_text = "DefineDoddy",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/DefineDoddy")
    end,
  })
  Button({
    group = self.credits,
    x = 140,
    y = 210,
    button_text = "Ge0force",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/Ge0forceBE")
    end,
  })
  Button({
    group = self.credits,
    ...
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/thecryru")
    end,
  })
  Button({
    group = self.credits,
    x = 258,
    y = 210,
    button_text = "Yongmin Park",
    fg_color = "redm5",
    bg_color = "red",
    credits_button = true,
    action = function(b)
      open_url(b, "https://twitter.com/yongminparks")
    end,
  })
end
...
function Arena:gain_gold()
  ...
  self.gold_gained =
    random:int(level_to_gold_gained[self.level][1], level_to_gold_gained[self.level][2])
  self.interest = math.min(math.floor(gold / 5), 5)
    + math.min((merchant and math.floor(gold / 10) or 0), 10)
...
function Arena:transition()
  ...
  TransitionEffect({
    ...
    x = self.player.x,
    y = self.player.y,
    color = state.dark_transitions and bg[-2] or self.color,
    transition_action = function(t)
      if self.level % 2 == 0 and self.shop_level < 5 then
        self.shop_xp = self.shop_xp + 1
        local max_xp = 0
        if self.shop_level == 1 then
          max_xp = 3
        elseif self.shop_level == 2 then
          max_xp = 4
        elseif self.shop_level == 3 then
          max_xp = 5
        elseif self.shop_level == 4 then
          max_xp = 6
        elseif self.shop_level == 5 then
          max_xp = 0
        end
        ...
        t.text:set_text({
          {
            text = "[nudge_down, "
              .. tostring(state.dark_transitions and "fg" or "bg")
              .. "]gold gained: "
              .. tostring(self.gold_gained or 0)
              .. " + "
              .. tostring(self.gold_picked_up or 0),
            font = pixul_font,
            alignment = "center",
            ...
            t.text:set_text({
              ...
              },
              {
                text = "[nudge_down, "
                  ...
                  .. "]total: "
                  .. tostring(
                    (self.gold_gained or 0) + (self.interest or 0) + (self.gold_picked_up or 0)
                  ),
                font = pixul_font,
                alignment = "center",
              },
    ...
    text = Text({
      ...
      {
        text = "[wavy_lower, " .. tostring(state.dark_transitions and "fg" or "bg") .. "]total: 0",
        font = pixul_font,
        alignment = "center",
      },
    }, global_text_tags),
  ...
  self.t:every(0.1, function()
    ...
    SpawnEffect({
      ...
      action = function(x, y)
        ...
        if
          random:bool(table.reduce(level_to_elite_spawn_weights[self.level], function(memo, v)
            return memo + v
          end))
        then
          local elite_type = level_to_elite_spawn_types[self.level][random:weighted_pick(
            unpack(level_to_elite_spawn_weights[self.level])
          )]
          Seeker({
            group = self.main,
            x = x,
...
function CharacterHP:draw()
  ...
  graphics.rectangle(
    ...
    2,
    2,
    self.parent.dead and bg[5]
      or (self.hfx.hit.f and fg[0] or _G[character_color_strings[self.parent.character]][-2]),
    2
  )
  ...
  if not self.parent.dead then
    graphics.line(
      ...
      self.y + 5,
      self.hfx.hit.f and fg[0] or _G[character_color_strings[self.parent.character]][-2],
      2
    )
  end
  graphics.pop()
  ...
  if state.cooldown_snake then
    if
      table.any(non_cooldown_characters, function(v)
    ...
    if not p.dead then
      ...
      graphics.line(
        ...
        p.y + 8,
        self.hfx.hit.f and fg[0] or _G[character_color_strings[p.character]][-2],
        2
      )
    end
    graphics.pop()
