...
function Seeker:init(args)
  ...
  if self.boss then
    ...
    if level == 0 then
      self.level = 25 * math.floor(self.level / 25)
    end

    if self.boss == "speed_booster" then
      self.color = green[0]:clone()
      self.t:every(8, function()
        if self.silenced or self.barbarian_stunned then
          return
        end
    ...
    elseif self.boss == "forcer" then
      ...
      self.t:every(6, function()
        ...
        self.prs = 0
        self.t:tween(0.05, self, { prs = 4 }, math.cubic_in_out, function()
          self.spring:pull(0.15)
        end)
        self.t:after(2 - 0.05 * 7, function()
          self.t:every_immediate(0.05, function()
            self.phidden = not self.phidden
          end, 7)
        end)
        self.color_transparent = Color(yellow[0].r, yellow[0].g, yellow[0].b, 0.08)
        self.t:every(0.08, function()
          HitParticle({ group = main.current.effects, x = self.px, y = self.py, color = yellow[0] })
        end, math.floor(2 / 0.08))
        self.vr = 0
        ...
        self.t:during(2, function(dt)
          ...
          for _, enemy in ipairs(enemies) do
            enemy:apply_steering_force(
              math.remap(enemy:distance_to_point(self.px, self.py), 0, 160, 400, 200),
              enemy:angle_to_point(self.px, self.py)
            )
          end
    ...
    elseif self.boss == "swarmer" then
      ...
      self.t:every(4, function()
        ...
        local enemies = table.select(
          main.current.main:get_objects_by_classes(main.current.enemies),
          function(v)
            return v.id ~= self.id and v:is(Seeker)
          end
        )
    ...
    elseif self.boss == "randomizer" then
      self.t:every_immediate(0.07, function()
          ...
          if enemy then
            ...
            for i = 1, n do
              EnemyProjectile({
                ...
                r = (i - 1) * math.pi / (n / 2),
                v = 125 + 5 * enemy.level,
                dmg = (1 + 0.15 * current_new_game_plus) * enemy.dmg,
              })
            end
          end
  ...
  elseif self.headbutter then
    ...
    self.t:every(function()
      ...
      if self.headbutt_charging or self.headbutting then
        return
      end
      ...
      self.t:tween(
        ...
        function()
          ...
          self.t:tween(
            0.25,
            self.color,
            { r = orange[0].r, b = orange[0].b, g = orange[0].g },
            math.linear
          )
  ...
  elseif self.tank then
    ...
    self.t:every({ 3 * n, 5 * n }, function()
      ...
      local enemy =
        self:get_closest_object_in_shape(Circle(self.x, self.y, 64), main.current.enemies)
      if enemy then
        wizard1:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
        HitCircle({
          group = main.current.effects,
        ...
        for i = 1, 3 do
          self.t:after(math.max(1 - self.level * 0.01, 0.25) * 0.15 * (i - 1), function()
            ...
            EnemyProjectile({
              ...
              color = fg[0],
              r = r,
              v = math.min(140 + 3.5 * self.level + 2 * current_new_game_plus, 300),
              dmg = (current_new_game_plus * 0.05 + 1) * self.dmg,
              source = "shooter",
            })
          end)
        end
  ...
  if player and player.intimidation and not self.boss and not self.tank then
    self.buff_hp_m = (player.intimidation == 1 and 0.9)
      or (player.intimidation == 2 and 0.8)
      or (player.intimidation == 3 and 0.7)
    self:calculate_stats()
    self.hp = self.max_hp
  end

  if player and player.vulnerability then
    self.vulnerable = (player.vulnerability == 1 and 1.1)
      or (player.vulnerability == 2 and 1.2)
      or (player.vulnerability == 3 and 1.3)
  end

  if player and player.temporal_chains then
    self.temporal_chains_mvspd_m = (player.temporal_chains and 0.9)
      or (player.temporal_chains and 0.8)
      or (player.temporal_chains and 0.7)
  end
...
function Seeker:update(dt)
  ...
  if main.current.mage_level == 2 then
    self.buff_def_a = -30
  elseif main.current.mage_level == 1 then
    self.buff_def_a = -15
  else
    self.buff_def_a = 0
  end
  ...
  if self.speed_boosting then
    local n = math.remap(
      love.timer.getTime() - self.speed_boosting,
      0,
      (3 + 0.015 * self.level + current_new_game_plus * 0.1),
      1,
      0.5
    )
    self.speed_boosting_mvspd_m = (3 + 0.015 * self.level + 0.1 * current_new_game_plus) * n
    if
      not self.speed_booster
      and not self.exploder
      and not self.headbutter
      and not self.tank
      and not self.shooter
      and not self.spawner
    then
  ...
  self.buff_mvspd_m = (self.speed_boosting_mvspd_m or 1)
    * (self.slow_mvspd_m or 1)
    * (self.temporal_chains_mvspd_m or 1)
    * (self.tank and 0.35 or 1)
    * (self.deceleration_mvspd_m or 1)
  ...
  if self.being_pushed then
    ...
    if self.taunted and self.taunted.dead then
      target = main.current.player
      self.taunted = nil
    end
...
function Seeker:draw()
  ...
  if self.boss then
    graphics.rectangle(
      ...
      4,
      4,
      self.hfx.hit.f and fg[0] or (self.silenced and bg[10]) or self.color
    )
  else
    graphics.rectangle(
      self.x,
  ...
  if self.boss then
    ...
    graphics.line(
      self.x - self.shape.w / 2,
      self.y + 8,
      self.x - self.shape.w / 2 + self.shape.w * n,
      self.y + 8,
      self.color,
      2
    )
...
function Seeker:on_collision_enter(other, contact)
  ...
  elseif table.any(main.current.enemies, function(v)
    ...
    elseif self.headbutting then
      ...
      hit2:play({ pitch = random:float(0.95, 1.05), volume = 0.35 })
      if other:is(Seeker) or other:is(Player) then
        self.headbutting = false
      end
    end
...
function Seeker:hit(damage, projectile, dot, from_enemy)
  ...
  if main.current.player.critical_strike and not dot and not from_enemy then
    if
      random:bool(
        (main.current.player.critical_strike == 1 and 5)
          or (main.current.player.critical_strike == 2 and 10)
          or (main.current.player.critical_strike == 3 and 15)
      )
    then
      crit = 2
  ...
  if main.current.player.kinetic_strike and not dot and not from_enemy then
    if
      random:bool(
        (main.current.player.kinetic_strike == 1 and 10)
          or (main.current.player.kinetic_strike == 2 and 20)
          or (main.current.player.kinetic_strike == 3 and 30)
      )
    then
  ...
  if main.current.player.stunning_strike and not dot and not from_enemy then
    if
      random:bool(
        (main.current.player.stunning_strike == 1 and 8)
          or (main.current.player.stunning_strike == 2 and 16)
          or (main.current.player.stunning_strike == 3 and 24)
      )
    then
      self:slow(0.1, 2)
      self.barbarian_stunned = true
      self.t:after(2, function()
        self.barbarian_stunned = false
      end)
    end
  end
  ...
  if main.current.player.silencing_strike and not dot and not from_enemy then
    if
      random:bool(
        (main.current.player.silencing_strike == 1 and 8)
          or (main.current.player.silencing_strike == 2 and 16)
          or (main.current.player.silencing_strike == 3 and 24)
      )
    then
      self.silenced = true
  ...
  if main.current.player.noxious_strike and not dot and not from_enemy then
    if
      random:bool(
        (main.current.player.noxious_strike == 1 and 8)
          or (main.current.player.noxious_strike == 2 and 16)
          or (main.current.player.noxious_strike == 3 and 24)
      )
    then
      self:apply_dot(
        0.6
          * actual_damage
          * (main.current.player.dot_dmg_m or 1)
  ...
  if dot then
    self.seeping_def_m = (main.current.player.seeping == 1 and 0.85)
      or (main.current.player.seeping == 2 and 0.75)
      or (main.current.player.seeping == 3 and 0.65)
      or 1
    self.t:after(1, function()
      self.seeping_def_m = 1
    end, "seeping")

    self.deceleration_mvspd_m = (main.current.player.deceleration == 1 and 0.85)
      or (main.current.player.deceleration == 2 and 0.75)
      or (main.current.player.deceleration == 3 and 0.65)
      or 1
    self.t:after(1, function()
  ...
  if self.boss and main.current.player.culling_strike then
    if
      self.hp
      <= self.max_hp
        * ((main.current.player.culling_strike == 1 and 0.1) or (main.current.player.culling_strike == 2 and 0.2) or (main.current.player.culling_strike == 3 and 0.3))
    then
      self.hp = 0
    end
  end
  ...
  if self.hp <= 0 then
    ...
    if pyrod then
      trigger:after(0.01, function()
        ...
        Area({
          ...
          y = self.y,
          color = red[0],
          w = 32 * pyrod.parent.area_size_m,
          r = random:float(0, 2 * math.pi),
          dmg = pyrod.parent.area_dmg_m * pyrod.dmg,
          character = pyrod.character,
          level = pyrod.level,
          parent = pyrod.parent,
        })
      end)
        ...
        if type(self.infested) == "number" then
          for i = 1, self.infested do
            Critter({
              ...
              r = random:float(0, 2 * math.pi),
              v = 10,
              dmg = self.infested_dmg,
              parent = self.infested_ref,
            })
          end
        end
      end)
      ...
      trigger:after(0.01, function()
        ...
        for i = 1, 4 do
          local t = {
            ...
            r = r,
            color = red[0],
            dmg = self.jester_ref.dmg,
            pierce = self.jester_lvl3 and 2 or 0,
            homing = self.jester_lvl3,
            character = self.jester_ref.character,
            parent = self.jester_ref,
          }
    ...
    if self.bane_cursed then
      trigger:after(0.01, function()
        DotArea({
          ...
          x = self.x,
          y = self.y,
          rs = (self.bane_ref.level == 3 and 2 or 1) * self.bane_ref.area_size_m * 27,
          color = purple[0],
    ...
    if main.current.player.homing_barrage and not from_enemy then
      trigger:after(0.01, function()
        if not main.current.player then
          return
        end
        if
          random:bool(
            (main.current.player.homing_barrage == 1 and 8)
              or (main.current.player.homing_barrage == 2 and 16)
              or (main.current.player.homing_barrage == 3 and 24)
          )
        then
          local target = main.current.player:get_closest_object_in_shape(
            Circle(main.current.player.x, main.current.player.y, 128),
            main.current.enemies
          )
          main.current.player:barrage(
            target and main.current.player:angle_to_object(target) or main.current.player.r,
            4,
            nil,
            nil,
            nil,
    ...
    if main.current.player.infesting_strike and not from_enemy then
      trigger:after(0.01, function()
        ...
        if
          random:bool(
            (main.current.player.infesting_strike == 1 and 10)
              or (main.current.player.infesting_strike == 2 and 20)
              or (main.current.player.infesting_strike == 3 and 30)
          )
        then
          critter1:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
          for i = 1, 2 do
...
function Seeker:curse(curse, duration, arg1, arg2, arg3)
  ...
  if curse == "launcher" then
    self.t:after(duration, function()
      ...
      self.launcher = arg2
      self:push(
        random:float(50, 75) * self.launcher.knockback_m,
        random:table({ 0, math.pi, math.pi / 2, -math.pi / 2 })
      )
    end, "launcher_curse")
  ...
  if main.current.player.whispers_of_doom then
    ...
    if
      self.doom
      == (
        (main.current.player.whispers_of_doom == 1 and 4)
        or (main.current.player.whispers_of_doom == 2 and 3)
        or (main.current.player.whispers_of_doom == 3 and 2)
      )
    then
      self.doom = 0
      self:hit(
        (main.current.player.whispers_of_doom == 1 and 100)
          or (main.current.player.whispers_of_doom == 2 and 150)
          or (main.current.player.whispers_of_doom == 3 and 200)
      )
      buff1:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
        ...
        if main.current.main.world then
          for i = 1, n do
            EnemyProjectile({
              ...
              color = blue[0],
              r = (i - 1) * math.pi / (n / 2),
              v = 120 + math.min(5 * self.parent.level, 300),
              dmg = 1.3 * self.parent.dmg,
            })
          end
        end
        self.dead = true
...
function EnemyCritter:hit(damage, projectile)
  ...
  if projectile == self.invulnerable_to then
    return
  end
  self.hfx:use("hit", 0.25, 200, 10)
...
function EnemyCritter:apply_dot(dmg, duration)
  self.t:every(0.25, function()
    ...
    for i = 1, 1 do
      HitParticle({ group = main.current.effects, x = self.x, y = self.y, color = purple[0] })
    end
  end, math.floor(duration / 0.2))
end
