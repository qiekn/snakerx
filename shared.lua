...
function SpawnEffect:init(args)
  ...
  self.t:tween(0.1, self, { rs = 6 }, math.cubic_in_out, function()
    ...
    self.t:tween(0.25, self, { rs = 0 }, math.linear, function()
      self.dead = true
    end)
...
function Nodemap:generate_nodemap(group, nodemap, color_mode)
  for id, node in pairs(nodemap) do
    Node({
      ...
      can_be_visited = node.can_be_visited,
      color = node.color,
      label = node.label,
      data = node.data,
      on_visit = node.on_visit,
      on_draw = node.on_draw,
...
function Node:init(args)
  ...
  self.t:after(0.01, function()
    ...
    for _, neighbor_id in ipairs(self.neighbors) do
      local edge = nil
      edge = self.group:get_object_by_properties(
        { "node1_id", "node2_id" },
...
function Node:update(dt)
  ...
  if self.color_mode == "skill_tree" then
    if self.can_be_visited then
      self.color = bg[10]
      if self.label then
        self.label_color = bg[10]
      end
      if self.hot then
        self.color = fg[0]
        if self.label then
          self.label_color = fg[0]
        end
      end
    end
  end
  ...
  if self.hot and self.can_be_visited and not self.visited and input.m1.pressed then
    ...
    if self.color_mode == "skill_tree" then
      ...
      if self.label then
        self.label_color = self.src_color
      end
    else
      self.color = bg[5]
      if self.label then
        self.label_color = bg[5]
      end
    end
    if self.label then
      self.label_color = self.color
    end
    if self.on_visit then
      self:on_visit()
    end
  end
end
...
function Node:draw()
  if self.on_draw then
    ...
    if self.hot and self.can_be_visited then
      graphics.circle(self.x, self.y, 1.15 * self.shape.rs, self.color)
    else
      graphics.circle(self.x, self.y, self.shape.rs, self.color, 3)
    end
    graphics.pop()
  end
  ...
  if self.label then
    ...
    graphics.print_centered(
      self.label,
      pixul_font,
      self.x + (w / 6) * math.cos(self.label_r) + s * self.rs * math.cos(self.label_r),
      self.y + s * self.rs * math.sin(self.label_r),
      0,
      self.spring.x * self.sx,
      self.spring.x * self.sy,
      nil,
      nil,
      self.label_color
    )
...
function Edge:init(args)
  ...
  self.x1, self.y1 =
    self.node1.x + 2.75 * self.node1.rs * math.cos(r),
    self.node1.y + 2.75 * self.node1.rs * math.sin(r)
  self.x2, self.y2 =
    self.node2.x + 2.75 * self.node2.rs * math.cos(r - math.pi),
    self.node2.y + 2.75 * self.node2.rs * math.sin(r - math.pi)
...
function Edge:update(dt)
  ...
  if self.color_mode == "skill" then
    self.color = bg[5]
    if
      (self.node1.visited and self.node2.can_be_visited)
      or (self.node2.visited and self.node1.can_be_visited)
    then
      self.color = bg[10]
    end
    if
      (self.node1.visited and self.node2.visited)
      or (self.node1.visited and self.node2.hot)
      or (self.node2.visited and self.node1.hot)
    then
      self.color = fg[0]
    end
  else
    if self.node1.visited and self.node2.visited then
      self.color = bg[5]
    end
  end
...
function HoverCrosshair:draw()
  ...
  graphics.polyline(
    ...
    self.line_width,
    self.x - self.ox,
    self.y - self.oy + 0.4 * self.h,
    self.x - self.ox,
    self.y - self.oy,
    self.x - self.ox + 0.4 * self.w,
    self.y - self.oy
  )
  graphics.polyline(
    fg[0],
...
function HoverCrosshair:activate(x, y, w, h, line_width)
  ...
  if self.w <= 10 and self.h <= 10 then
    self.animation = AnimationLogic(0.075, 3, "bounce", {
      function()
        self.ox, self.oy = 0.6 * self.w, 0.6 * self.h
      end,
      function()
        self.ox, self.oy = 0.8 * self.w, 0.8 * self.h
      end,
      function()
        self.ox, self.oy = self.w, self.h
      end,
    })
  else
    self.animation = AnimationLogic(0.075, 3, "bounce", {
      ...
      function()
        self.ox, self.oy = 0.9 * self.w, 0.9 * self.h
      end,
      function()
        self.ox, self.oy = self.w, self.h
      end,
...
function HoverCrosshair:deactivate()
  ...
  self.t:tween(0.05, self, { sx = 0, sy = 0 }, math.linear, function()
    self.sx, self.sy = 0, 0
  end, "deactivate")
end
...
function InfoText:activate(text, ox, oy, sx, sy, ow, oh, tox, toy)
  ...
  self.t:cancel("deactivate")
  self.t:tween(0.1, self, { sx = sx or 1, sy = sy or 1 }, math.cubic_in_out, function()
    self.sx, self.sy = sx or 1, sy or 1
  end, "activate")
...
function InfoText:deactivate()
  self.t:cancel("activate")
  self.t:tween(0.05, self, { sy = 0 }, math.linear, function()
    self.sy = 0
    self.dead = true
  end, "deactivate")
end
...
function RefreshEffect:init(args)
  self:init_game_object(args)
  self.oy = self.h / 3
  self.t:tween(0.15, self, { h = 0 }, math.linear, function()
    self.dead = true
  end)
end
...
function flash(duration, color)
  ...
  trigger:after(duration, function()
    flashing = false
  end, "flash")
end
...
function AnimationEffect:draw()
  self.animation:draw(
    ...
    self.y + (self.ox or 0),
    self.r + (self.oa or 0),
    (self.flip_sx or 1) * self.sx,
    (self.flip_sy or 1) * self.sy,
    nil,
    nil,
    self.color
  )
...
function AnimationEffect:set_linear_movement(v, r)
  ...
  self.t:after(2 * duration / 3, function()
    self.t:tween(duration / 3, self, { v = 0 }, math.cubic_in_out)
  end)
end
