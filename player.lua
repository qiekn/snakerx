...
function Player:init(args)
  ...
  elseif self.character == "gambler" then
    ...
    local cast = function(pitch_a)
      ...
      if enemy then
        gambler1:play({
          pitch = pitch_a,
          volume = math.clamp(math.remap(gold, 0, 50, 0, 0.5), 0, 0.75),
        })
        enemy:hit(2 * gold)
        if main.current.sorcerer_level > 0 then
          ...
          if
            self.sorcerer_count
            >= (
              (main.current.sorcerer_level == 3 and 2)
              or (main.current.sorcerer_level == 2 and 3)
              or (main.current.sorcerer_level == 1 and 4)
            )
          then
            ...
            self.t:after(0.25, function()
              local enemy =
                table.shuffle(main.current.main:get_objects_by_classes(main.current.enemies))[1]
              if enemy then
                gambler1:play({
                  pitch = pitch_a + 0.05,
                  volume = math.clamp(math.remap(gold, 0, 50, 0, 0.5), 0, 0.75),
  ...
  elseif self.character == "archer" then
    ...
    self.t:cooldown(2, function()
      ...
      if closest_enemy then
        self:shoot(
          self:angle_to_object(closest_enemy),
          { pierce = 1000, ricochet = (self.level == 3 and 3 or 0) }
        )
      end
    end, nil, nil, "shoot")
    ...
    self.t:every(8, function()
      if self.level == 3 then
        for i = 1, 4 do
          ...
          local objects = main.current.main:get_objects_in_shape(
            check_circle,
            {
              Seeker,
              EnemyCritter,
          ...
          while #objects > 0 do
            ...
            objects = main.current.main:get_objects_in_shape(
              check_circle,
              {
                Seeker,
                EnemyCritter,
        ...
        action = function(x, y)
          ...
          if #objects == 0 then
            Automaton({
              ...
              parent = self,
              level = self.level,
              conjurer_buff_m = self.conjurer_buff_m or 1,
            })
          end
        end,
      })
        ...
        if enemy then
          if self.level == 3 then
            ...
            self.t:after(0.5, function()
              local enemy =
                self:get_random_object_in_shape(self.attack_sensor, main.current.enemies)
              if enemy then
                self:attack(32, { x = enemy.x, y = enemy.y })
              end
            end)
        ...
        SpawnEffect({
          ...
          action = function(x, y)
            Saboteur({
              ...
              level = self.level,
              conjurer_buff_m = self.conjurer_buff_m or 1,
              crit = (self.level == 3) and random:bool(50),
            })
          end,
        })
      end, 2)
    ...
    self.t:every(12, function()
      local volcano = function()
        ...
        main.current.t:every_immediate(0.1, function()
          ...
          local objects = main.current.main:get_objects_in_shape(
            check_circle,
            { Player, Seeker, EnemyCritter, Critter, Saboteur, Pet, Turret, Sentry, Bomb }
          )
          if #objects == 0 then
            Volcano({
              group = main.current.main,
  ...
  elseif self.character == "plague_doctor" then
    ...
    if self.level == 3 then
      self.t:after(0.01, function()
        self.dot_area = DotArea({
          group = main.current.effects,
          x = self.x,
          y = self.y,
          rs = self.area_size_m * 48,
          color = self.color,
  ...
  elseif self.character == "lich" then
    ...
    self.t:cooldown(4, function()
      ...
      if closest_enemy then
        self:shoot(
          self:angle_to_object(closest_enemy),
          { chain = (self.level == 3 and 14 or 7), v = 140 }
        )
      end
    end, nil, nil, "shoot")
  ...
  elseif self.character == "corruptor" then
    ...
    self.t:cooldown(2, function()
      ...
      if closest_enemy then
        self:shoot(
          self:angle_to_object(closest_enemy),
          { spawn_critters_on_kill = 3, spawn_critters_on_hit = (self.level == 3 and 2 or nil) }
        )
      end
    end, nil, nil, "shoot")
    ...
    self.t:cooldown(6, function()
      ...
      for _, enemy in ipairs(enemies) do
        if self:distance_to_object(enemy) < 128 then
          ...
          if self.resonance then
            resonance_dmg = (
              self.level == 3 and 6 * self.dmg * 0.05 * #enemies or 2 * self.dmg * 0.05 * #enemies
            )
          end
          enemy:curse(
            "launcher",
            4 * (self.hex_duration_m or 1),
            (self.level == 3 and 6 * self.dmg or 2 * self.dmg) + resonance_dmg,
            self
          )
          HitCircle({
            group = main.current.effects,
  ...
  elseif self.character == "jester" then
    ...
    self.t:cooldown(6, function()
      ...
      buff1:play({ pitch = random:float(0.9, 1.1), volume = 0.5 })
      local enemies = table.first2(
        table.shuffle(self:get_objects_in_shape(self.wide_attack_sensor, main.current.enemies)),
        6
          + ((self.malediction == 1 and 1) or (self.malediction == 2 and 3) or (self.malediction == 3 and 5) or 0)
  ...
  elseif self.character == "usurer" then
    ...
    self.t:cooldown(6, function()
      ...
      for _, enemy in ipairs(enemies) do
        ...
        enemy:apply_dot(
          self.dmg * (self.dot_dmg_m or 1) * (main.current.chronomancer_dot or 1),
          10000
        )
        HitCircle({
          group = main.current.effects,
      ...
      local curse = function()
        ...
        local enemies = table.first2(
          ...
          6
            + ((self.malediction == 1 and 1) or (self.malediction == 2 and 3) or (self.malediction == 3 and 5) or 0)
            + (
              (main.current.curser_level == 2 and 3)
              or (main.current.curser_level == 1 and 1)
              or 0
            )
        )
        for _, enemy in ipairs(enemies) do
  ...
  elseif self.character == "psykino" then
    self.t:every(4, function()
      local center_enemy =
        self:get_random_object_in_shape(Circle(self.x, self.y, 160), main.current.enemies)
      if center_enemy then
        ForceArea({
          group = main.current.effects,
          x = center_enemy.x,
          y = center_enemy.y,
          rs = self.area_size_m * 64,
          color = self.color,
          character = self.character,
          level = self.level,
          parent = self,
      ...
      if self.barrager_counter == 3 and self.level == 3 then
        ...
        for i = 1, 15 do
          self.t:after((i - 1) * 0.05, function()
            self:shoot(
              r + random:float(-math.pi / 32, math.pi / 32),
              { knockback = (self.level == 3 and 14 or 7) }
            )
          end)
        end
      else
  ...
  elseif self.character == "fairy" then
    self.t:every(6, function()
      if self.level == 3 then
        ...
        if unit_1 then
          while
            table.any(non_attacking_characters, function(v)
              return v == unit_1.character
            end) and runs < 1000
          do
        ...
        if unit_1 then
          ...
          unit_1.fairyd = true
          unit_1.t:after(5.98, function()
            unit_1.fairy_aspd_m = 1
            unit_1.fairyd = false
          end)
        end
        if unit_2 then
          ...
          unit_2.fairyd = true
          unit_2.t:after(5.98, function()
            unit_2.fairy_aspd_m = 1
            unit_2.fairyd = false
          end)
        end
        heal1:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
        ...
        if self.level == 3 then
          ...
          if unit_1 then
            ...
            main.current.t:every_immediate(0.1, function()
              ...
              local objects =
                main.current.main:get_objects_in_shape(check_circle, { Seeker, EnemyCritter })
              if #objects == 0 then
  ...
  if self.ouroboros_technique_r then
    self.t:after(0.01, function()
      self.t:every(
        (self.ouroboros_technique_r == 1 and 0.5)
          or (self.ouroboros_technique_r == 2 and 0.33)
          or (self.ouroboros_technique_r == 3 and 0.25),
        function()
          if
            self.leader
              and (state.mouse_control and table.all(self.mouse_control_v_buffer, function(v)
                return v >= 0.5
              end))
            or (self.move_right_pressed and love.timer.getTime() - self.move_right_pressed > 1)
          then
            local target =
  ...
  if self.centipede then
    self.centipede_mvspd_m = (self.centipede == 1 and 1.1)
      or (self.centipede == 2 and 1.2)
      or (self.centipede == 3 and 1.3)
  end
  if self.amplify then
    self.amplify_area_dmg_m = (self.amplify == 1 and 1.2)
      or (self.amplify == 2 and 1.35)
      or (self.amplify == 3 and 1.5)
  end

  if self.ballista and launches_projectiles(self.character) then
    self.ballista_dmg_m = (self.ballista == 1 and 1.2)
      or (self.ballista == 2 and 1.35)
      or (self.ballista == 3 and 1.5)
  end

  if self.chronomancy then
    if table.any(self.classes, function(v)
      return v == "mage"
    end) then
      self.chronomancy_aspd_m = (self.chronomancy == 1 and 1.15)
        or (self.chronomancy == 2 and 1.25)
        or (self.chronomancy == 3 and 1.35)
    end
  end
  ...
  if self.leader and self.awakening then
    main.current.t:after(0.1, function()
      ...
      local mages = {}
      for _, unit in ipairs(units) do
        if table.any(unit.classes, function(v)
          return v == "mage"
        end) then
      ...
      if mage then
        ...
        while
          table.any(non_attacking_characters, function(v)
            return v == mage.character
          end) and runs < 1000
        do
          mage = random:table(mages)
          runs = runs + 1
        end
        mage.awakening_aspd_m = (self.awakening == 1 and 1.5)
          or (self.awakening == 2 and 1.75)
          or (self.awakening == 3 and 2)
        mage.awakening_dmg_m = (self.awakening == 1 and 1.5)
          or (self.awakening == 2 and 1.75)
          or (self.awakening == 3 and 2)
      end
    end)
  ...
  if self.leader and self.divine_punishment then
    main.current.t:every(5, function()
      ...
      for _, enemy in ipairs(enemies) do
        ...
        LightningLine({
          group = main.current.effects,
          src = { x = enemy.x, y = enemy.y - 32 },
          dst = enemy,
          color = blue[0],
          duration = 0.2,
        })
        _G[random:table({ "spark1", "spark2", "spark3" })]:play({
  ...
  if
    self.unwavering_stance and table.any(self.classes, function(v)
      return v == "warrior"
    end)
  then
    ...
    self.t:every(5, function()
      self.unwavering_stance_def_m = self.unwavering_stance_def_m
        + (
          (self.unwavering_stance == 1 and 0.04)
          or (self.unwavering_stance == 2 and 0.08)
          or (self.unwavering_stance == 3 and 0.12)
        )
    end)
  end

  if self.magnify then
    self.magnify_area_size_m = (self.magnify == 1 and 1.2)
      or (self.magnify == 2 and 1.35)
      or (self.magnify == 3 and 1.5)
  end

  if self.unleash and table.any(self.classes, function(v)
    return v == "nuker"
  end) then
  ...
  if self.reinforce then
    main.current.t:after(0.1, function()
      ...
      if any_enchanter then
        local v = (self.reinforce == 1 and 1.1)
          or (self.reinforce == 2 and 1.2)
          or (self.reinforce == 3 and 1.3)
          or 1
    ...
    main.current.t:after(0.1, function()
      ...
      if enchanter_amount >= 2 then
        ...
        if unit then
          ...
          unit.enchanted_aspd_m = (self.enchanted == 1 and 1.33)
            or (self.enchanted == 2 and 1.66)
            or (self.enchanted == 3 and 1.99)
        end
      end
    end)
  end
  ...
  if self.leader and self.immolation then
    main.current.t:after(0.1, function()
      ...
      if unit_1 then
        unit_1.t:every(2, function()
          unit_1:hit(0.05 * unit_1.max_hp)
        end)
      end
      if unit_2 then
        unit_2.t:every(2, function()
          unit_2:hit(0.05 * unit_2.max_hp)
        end)
      end
      if unit_3 then
        unit_3.t:every(2, function()
          unit_3:hit(0.05 * unit_3.max_hp)
        end)
      end
      local units = self:get_all_units()
      for _, unit in ipairs(units) do
  ...
  if self.leader and self.shoot_5 then
    main.current.t:after(0.1, function()
      main.current.t:every(0.33, function()
        ...
        if unit then
          local target =
            unit:get_closest_object_in_shape(Circle(unit.x, unit.y, 96), main.current.enemies)
          if target then
    ...
    self.t:after(1, function()
      ...
      for _, unit in ipairs(units) do
        if table.any(unit.classes, function(v)
          ...
          Projectile({
            group = main.current.main,
            x = unit.x + 24 * math.cos(unit.r),
            y = unit.y + 24 * math.sin(unit.r),
            color = fg[0],
            v = 200,
            dmg = unit.dmg,
            character = "psyker",
...
function Player:update(dt)
  ...
  if self.character == "vagrant" and self.level == 3 then
    ...
    if class_levels.ranger >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.warrior >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.mage >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.rogue >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.healer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.conjurer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.enchanter >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.psyker >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.nuker >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.curser >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.forcer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.swarmer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.voider >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.sorcerer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.mercenary >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
    if class_levels.explorer >= 1 then
      number_of_active_sets = number_of_active_sets + 1
    end
  ...
  if table.any(self.classes, function(v)
    ...
    if main.current.ranger_level == 2 then
      self.chance_to_barrage = 16
    elseif main.current.ranger_level == 1 then
      self.chance_to_barrage = 8
    elseif main.current.ranger_level == 0 then
      self.chance_to_barrage = 0
    end
  end
  ...
  if table.any(self.classes, function(v)
    ...
    if main.current.warrior_level == 2 then
      self.warrior_def_a = 50
    elseif main.current.warrior_level == 1 then
      self.warrior_def_a = 25
    elseif main.current.warrior_level == 0 then
      self.warrior_def_a = 0
    end
  end

  self.heal_effect_m = 1
  if self.blessing then
    self.heal_effect_m = self.heal_effect_m
      * (
        (self.blessing == 1 and 1.1)
        or (self.blessing == 2 and 1.2)
        or (self.blessing == 3 and 1.3)
      )
  end
  ...
  if table.any(self.classes, function(v)
    ...
    if main.current.nuker_level == 2 then
      self.nuker_area_size_m = 1.25
      self.nuker_area_dmg_m = 1.25
    elseif main.current.nuker_level == 1 then
      self.nuker_area_size_m = 1.15
      self.nuker_area_dmg_m = 1.15
    elseif main.current.nuker_level == 0 then
      self.nuker_area_size_m = 1
      self.nuker_area_dmg_m = 1
    end
  end

  if main.current.conjurer_level == 2 then
    self.conjurer_buff_m = 1.5
  elseif main.current.conjurer_level == 1 then
    self.conjurer_buff_m = 1.25
  else
    self.conjurer_buff_m = 1
  end

  if table.any(self.classes, function(v)
    return v == "rogue"
  end) then
    if main.current.rogue_level == 2 then
      self.chance_to_crit = 30
    elseif main.current.rogue_level == 1 then
      self.chance_to_crit = 15
    elseif main.current.rogue_level == 0 then
      self.chance_to_crit = 0
    end
  end

  if main.current.enchanter_level == 2 then
    self.enchanter_dmg_m = 1.25
  elseif main.current.enchanter_level == 1 then
    self.enchanter_dmg_m = 1.15
  else
    self.enchanter_dmg_m = 1
  end
  ...
  if main.current.forcer_level == 2 then
    self.knockback_m = 1.5
  elseif main.current.forcer_level == 1 then
    self.knockback_m = 1.25
  else
    self.knockback_m = 1
  end
  if self.force_push then
    self.knockback_m = self.knockback_m * 1.25
  end
  ...
  if table.any(self.classes, function(v)
    ...
    if main.current.voider_level == 2 then
      self.dot_dmg_m = 1.4
    elseif main.current.voider_level == 1 then
      self.dot_dmg_m = 1.2
    else
      self.dot_dmg_m = 1
    end
  ...
  if self.call_of_the_void then
    self.dot_dmg_m = (self.dot_dmg_m or 1)
      * (
        (self.call_of_the_void == 1 and 1.3)
        or (self.call_of_the_void == 2 and 1.6)
        or (self.call_of_the_void == 3 and 1.9)
        or 1
      )
  end
  ...
  if self.ouroboros_technique_l and self.leader then
    ...
    if
      (
        state.mouse_control
        and table.all(self.mouse_control_v_buffer, function(v)
          return v <= -0.5
        end)
      ) or (self.move_left_pressed and love.timer.getTime() - self.move_left_pressed > 1)
    then
      for _, unit in ipairs(units) do
        unit.ouroboros_def_m = (self.ouroboros_technique_l == 1 and 1.15)
          or (self.ouroboros_technique_l == 2 and 1.25)
          or (self.ouroboros_technique_l == 3 and 1.35)
      end
    else
      for _, unit in ipairs(units) do
  ...
  if self.berserking and table.any(self.classes, function(v)
    return v == "warrior"
  end) then
    self.berserking_aspd_m = math.remap(
      self.hp / self.max_hp,
      0,
      1,
      (self.berserking == 1 and 1.5)
        or (self.berserking == 2 and 1.75)
        or (self.berserking == 3 and 2),
      1
    )
  end
  ...
  if self.defensive_stance and self.leader then
    self.defensive_stance_def_m = (self.defensive_stance == 1 and 1.1)
      or (self.defensive_stance == 2 and 1.2)
      or (self.defensive_stance == 3 and 1.3)
  end

  if
    self.defensive_stance
    and not self.leader
    and self.follower_index == #self.parent.followers
  then
    self.defensive_stance_def_m = (self.defensive_stance == 1 and 1.1)
      or (self.defensive_stance == 2 and 1.2)
      or (self.defensive_stance == 3 and 1.3)
  ...
  if self.offensive_stance and self.leader then
    self.offensive_stance_dmg_m = (self.offensive_stance == 1 and 1.1)
      or (self.offensive_stance == 2 and 1.2)
      or (self.offensive_stance == 3 and 1.3)
  end

  if
    self.offensive_stance
    and not self.leader
    and self.follower_index == #self.parent.followers
  then
    self.offensive_stance_dmg_m = (self.offensive_stance == 1 and 1.1)
  ...
  if self.dividends and table.any(self.classes, function(v)
    return v == "mercenary"
  end) then
  ...
  if self.haste then
    if self.hasted then
      self.haste_mvspd_m =
        math.clamp(math.remap(love.timer.getTime() - self.hasted, 0, 4, 1.5, 1), 1, 1.5)
    else
  ...
  self.buff_aspd_m = (self.chronomancer_aspd_m or 1)
    * (self.vagrant_aspd_m or 1)
    * (self.outlaw_aspd_m or 1)
    * (self.fairy_aspd_m or 1)
    * (self.psyker_aspd_m or 1)
    * (self.chronomancy_aspd_m or 1)
    * (self.awakening_aspd_m or 1)
    * (self.berserking_aspd_m or 1)
    * (self.reinforce_aspd_m or 1)
    * (self.squire_aspd_m or 1)
    * (self.speed_3_aspd_m or 1)
    * (self.last_stand_aspd_m or 1)
    * (self.enchanted_aspd_m or 1)
    * (self.explorer_aspd_m or 1)
    * (self.magician_aspd_m or 1)
  self.buff_dmg_m = (self.squire_dmg_m or 1)
    * (self.vagrant_dmg_m or 1)
    * (self.enchanter_dmg_m or 1)
    * (self.swordsman_dmg_m or 1)
    * (self.flagellant_dmg_m or 1)
    * (self.psyker_dmg_m or 1)
    ...
    * (self.damage_4_dmg_m or 1)
    * (self.offensive_stance_dmg_m or 1)
    * (self.last_stand_dmg_m or 1)
    * (self.dividends_dmg_m or 1)
    * (self.explorer_dmg_m or 1)
  self.buff_def_m = (self.squire_def_m or 1)
    * (self.ouroboros_def_m or 1)
    * (self.unwavering_stance_def_m or 1)
    * (self.reinforce_def_m or 1)
    * (self.defensive_stance_def_m or 1)
    * (self.last_stand_def_m or 1)
    * (self.unrelenting_stance_def_m or 1)
    * (self.hardening_def_m or 1)
  self.buff_area_size_m = (self.nuker_area_size_m or 1)
    * (self.magnify_area_size_m or 1)
    * (self.unleash_area_size_m or 1)
    * (self.last_stand_area_size_m or 1)
  self.buff_area_dmg_m = (self.nuker_area_dmg_m or 1)
    * (self.amplify_area_dmg_m or 1)
    * (self.unleash_area_dmg_m or 1)
    * (self.last_stand_area_dmg_m or 1)
  self.buff_mvspd_m = (self.wall_rider_mvspd_m or 1)
    * (self.centipede_mvspd_m or 1)
    * (self.squire_mvspd_m or 1)
    * (self.last_stand_mvspd_m or 1)
    * (self.haste_mvspd_m or 1)
  ...
  if self.leader then
    if not main.current:is(MainMenu) then
      ...
      if state.mouse_control then
        self.mouse_control_v = Vector(math.cos(self.r), math.sin(self.r)):perpendicular():dot(
          Vector(math.cos(self:angle_to_mouse()), math.sin(self:angle_to_mouse()))
        )
...
function Player:draw()
  graphics.push(
    ...
    self.y,
    self.r,
    self.hfx.hit.x * self.hfx.shoot.x,
    self.hfx.hit.x * self.hfx.shoot.x
  )
...
function Player:on_collision_enter(other, contact)
  ...
  if other:is(Wall) then
    if self.leader then
      ...
      if self.wall_rider then
        local units = self:get_all_units()
        for _, unit in ipairs(units) do
          unit.wall_rider_mvspd_m = 1.25
        end
        trigger:after(1, function()
          for _, unit in ipairs(units) do
            unit.wall_rider_mvspd_m = 1
          end
        end, "wall_rider")
      end
    end
  elseif table.any(main.current.enemies, function(v)
    ...
    if self.character == "vagrant" or self.character == "psykeeper" then
      other:hit(2 * self.dmg)
    else
      other:hit(self.dmg)
    end
...
function Player:hit(damage, from_undead)
  ...
  if self.undead and not from_undead then
    return
  end
  ...
  _G[random:table({ "player_hit1", "player_hit2" })]:play({
  ...
  if self.payback and table.any(self.classes, function(v)
    return v == "enchanter"
  end) then
    local units = self:get_all_units()
    for _, unit in ipairs(units) do
      if not unit.payback_dmg_m then
        unit.payback_dmg_m = 1
      end
      unit.payback_dmg_m = unit.payback_dmg_m
        + (
          (self.payback == 1 and 0.02)
          or (self.payback == 2 and 0.05)
          or (self.payback == 3 and 0.08)
          or 0
        )
    end
  end
  ...
  if
    self.unrelenting_stance and table.any(self.classes, function(v)
      return v == "warrior"
    end)
  then
    local units = self:get_all_units()
    for _, unit in ipairs(units) do
      if not unit.unrelenting_stance_def_m then
        unit.unrelenting_stance_def_m = 1
      end
      unit.unrelenting_stance_def_m = unit.unrelenting_stance_def_m
        + (
          (self.unrelenting_stance == 1 and 0.02)
          or (self.unrelenting_stance == 2 and 0.05)
          or (self.unrelenting_stance == 3 and 0.08)
          or 0
        )
    end
  end
  ...
  if self.crucio then
    ...
    for _, enemy in ipairs(enemies) do
      enemy:hit(
        ((self.crucio == 1 and 0.2) or (self.crucio == 2 and 0.3) or (self.crucio == 3 and 0.4))
          * actual_damage
      )
      HitCircle({
        group = main.current.effects,
    ...
    elseif self.lasting_7 and self.follower_index == 6 and not self.undead then
      ...
      if self.hardening then
        ...
        for _, unit in ipairs(units) do
          ...
          unit.t:after(3, function()
            unit.hardening_def_m = 1
          end)
        end
      end

      if
        self.annihilation and table.any(self.classes, function(v)
          return v == "voider"
        end)
      then
      ...
      if self.insurance then
        if
          random:bool(
            4
              * (
                (main.current.mercenary_level == 2 and 16)
                or (main.current.mercenary_level == 1 and 8)
                or 0
              )
          )
        then
  ...
  if enemy then
    if self.gravity_field then
      ForceArea({
        ...
        x = enemy.x,
        y = enemy.y,
        rs = self.area_size_m * 24,
        color = fg[0],
        character = "gravity_field",
        parent = self,
      })
    end
  end
  ...
  if enemy then
    if self.burning_field then
      ...
      DotArea({
        ...
        rs = self.area_size_m * 24,
        color = red[0],
        dmg = 30 * self.area_dmg_m * (self.dot_dmg_m or 1),
        duration = 2,
        character = "burning_field",
      })
    end
  end
  ...
  if enemy then
    if self.freezing_field then
      ...
      Area({
        ...
        x = enemy.x,
        y = enemy.y,
        w = self.area_size_m * 36,
        color = blue[0],
        character = "freezing_field",
        parent = self,
      })
    end
  end
...
function Player:chain_infuse(duration)
  ...
  self.t:after(duration or 2, function()
    self.chain_infused = false
  end, "chain_infuse")
end
  ...
  if self.dead then
    ...
    new_leader.t:every(0.01, function()
      table.insert(
        new_leader.previous_positions,
        1,
        { x = new_leader.x, y = new_leader.y, r = new_leader.r }
      )
...
function Player:shoot(r, mods)
  ...
  local dmg_m = 1
  local crit = false
  if self.character == "beastmaster" then
    crit = random:bool(10)
  end
  if self.chance_to_crit and random:bool(self.chance_to_crit) then
    dmg_m = (
      (self.assassination == 1 and 8)
      or (self.assassination == 2 and 10)
      or (self.assassination == 3 and 12)
      or 4
    )
    crit = true
  end
  if self.assassination and table.any(self.classes, function(v)
    return v == "rogue"
  end) then
    if not crit then
      dmg_m = 0.5
  ...
  elseif self.character == "dual_gunner" then
    HitCircle({
      group = main.current.effects,
      x = self.x + 0.8 * self.shape.w * math.cos(r) + 4 * math.cos(r - math.pi / 2),
      y = self.y + 0.8 * self.shape.w * math.sin(r) + 4 * math.sin(r - math.pi / 2),
      rs = 6,
    })
    HitCircle({
      group = main.current.effects,
    ...
    local t1 = {
      group = main.current.main,
      x = self.x + 1.6 * self.shape.w * math.cos(r) + 4 * math.cos(r - math.pi / 2),
      y = self.y + 1.6 * self.shape.w * math.sin(r) + 4 * math.sin(r - math.pi / 2),
      v = 300,
      r = r,
      color = self.color,
      dmg = self.dmg * dmg_m,
    ...
    if self.dg_counter == 5 and self.level == 3 then
      ...
      self.t:every(0.1, function()
        ...
        if random_enemy then
          _G[random:table({ "gun_kata1", "gun_kata2" })]:play({
            pitch = random:float(0.95, 1.05),
            volume = 0.35,
          })
          camera:spring_shake(2, r)
  ...
  elseif
    ...
    or self.character == "hunter"
    or self.character == "barrager"
    or self.character == "corruptor"
  then
    archer1:play({ pitch = random:float(0.95, 1.05), volume = 0.35 })
  ...
  elseif
    self.character == "scout"
    or self.character == "outlaw"
    or self.character == "blade"
    or self.character == "spellblade"
    or self.character == "jester"
    or self.character == "assassin"
    or self.character == "beastmaster"
    or self.character == "thief"
  then
  ...
  elseif self.character == "cannoneer" then
    _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
      pitch = random:float(0.95, 1.05),
      volume = 0.5,
    })
  end
...
function Player:attack(area, mods)
  ...
  then
    _G[random:table({ "swordsman1", "swordsman2" })]:play({
      pitch = random:float(0.9, 1.1),
      volume = 0.75,
    })
  elseif self.character == "elementor" then
...
function Player:dot_attack(area, mods)
  ...
  local t = {
    ...
    y = mods.y or self.y,
    r = self.r,
    rs = self.area_size_m * (area or 64),
    color = self.color,
    dmg = self.area_dmg_m * self.dmg * (self.dot_dmg_m or 1),
    character = self.character,
    level = self.level,
    parent = self,
  }
...
function Player:barrage(r, n, pierce, ricochet, shoot_5, homing)
  ...
  for i = 1, n do
    self.t:after((i - 1) * 0.075, function()
      if shoot_5 then
        archer1:play({ pitch = random:float(0.95, 1.05), volume = 0.2 })
      else
        archer1:play({ pitch = random:float(0.95, 1.05), volume = 0.35 })
      end
      HitCircle({
...
function Projectile:init(args)
  ...
  if self.character == "sage" then
    ...
    self.rs = 0
    self.t:tween(0.05, self, { rs = self.shape.w / 2.5 }, math.cubic_in_out, function()
      self.spring:pull(0.15)
    end)
    self.t:after(4, function()
      self.t:every_immediate(
        0.05,
        function()
        ...
        function()
          self:die()
          if self.level == 3 then
            _G[random:table({ "saboteur_hit1", "saboteur_hit2" })]:play({
              pitch = random:float(0.95, 1.05),
              volume = 0.2,
            })
  ...
  elseif self.character == "psyker" then
    ...
    self.orbit_speed = random:float(2, 4)
      * ((self.parent.orbitism == 1 and 1.25) or (self.parent.orbitism == 2 and 1.50) or (self.parent.orbitism == 3 and 1.75) or 1)
      * (1 / self.parent.aspd_m)
    self.orbit_offset = random:float(0, 2 * math.pi)
    self.dmg = self.dmg
      * (
        (self.parent.psychosink == 1 and 1.4)
        or (self.parent.psychosink == 2 and 1.8)
        or (self.parent.psychosink == 3 and 2.2)
        or 1
      )
  elseif self.character == "lich" then
    self.spring:pull(0.15)
  ...
  elseif self.character == "arcanist" then
    ...
    self.t:every(0.08, function()
      HitParticle({
        ...
        y = self.y,
        color = self.color,
        r = self.r + math.pi + random:float(-math.pi / 6, math.pi / 6),
        v = random:float(10, 25),
        parent = self,
      })
    end)
    self.t:every(self.parent.level == 3 and 0.54 or 0.8, function()
      local enemies = table.head(
        self:get_objects_in_shape(Circle(self.x, self.y, 128), main.current.enemies),
        self.level == 3 and 2 or 1
      )
      for _, enemy in ipairs(enemies) do
  ...
  if
    self.parent.divine_machine_arrow
    and table.any(self.parent.classes, function(v)
      return v == "ranger"
    end)
  then
    if
      random:bool(
        (self.parent.divine_machine_arrow == 1 and 10)
          or (self.parent.divine_machine_arrow == 2 and 20)
          or (self.parent.divine_machine_arrow == 3 and 30)
      )
    then
      self.homing = true
  ...
  if
    self.parent.blunt_arrow and table.any(self.parent.classes, function(v)
      return v == "ranger"
    end)
  then
    if
      random:bool(
        (self.parent.blunt_arrow == 1 and 10)
          or (self.parent.blunt_arrow == 2 and 20)
          or (self.parent.blunt_arrow == 3 and 30)
      )
    then
      self.knockback = 10
    end
  ...
  then
    self.chain = self.chain
      + (
        (self.parent.flying_daggers == 1 and 2)
        or (self.parent.flying_daggers == 2 and 3)
        or (self.parent.flying_daggers == 3 and 4)
      )
  end
end
...
function Projectile:update(dt)
  ...
  if self.character == "psyker" then
    ...
    self:set_position(
      self.parent.x
        + self.orbit_distance * math.cos(self.orbit_speed * main.current.t.time + self.orbit_offset),
      self.parent.y
        + self.orbit_distance * math.sin(self.orbit_speed * main.current.t.time + self.orbit_offset)
    )
  ...
  if self.character == "sage" then
    ...
    for _, enemy in ipairs(enemies) do
      enemy:apply_steering_force(
        math.remap(self:distance_to_object(enemy), 0, 100, 250, 50),
        enemy:angle_to_object(self)
      )
    end
...
function Projectile:die(x, y, r, n)
  ...
  if self.character == "wizard" then
    Area({
      ...
      y = self.y,
      r = self.r,
      w = self.parent.area_size_m * 24,
      color = self.color,
      dmg = self.parent.area_dmg_m * self.dmg,
      character = self.character,
      level = self.level,
      parent = self,
      void_rift = self.parent.void_rift,
      echo_barrage = self.parent.echo_barrage,
    })
  elseif self.character == "blade" then
    Area({
      group = main.current.effects,
  ...
  elseif self.character == "cannoneer" then
    Area({
      ...
      y = self.y,
      r = self.r,
      w = self.parent.area_size_m * 96,
      color = self.color,
      dmg = 2 * self.parent.area_dmg_m * self.dmg,
      character = self.character,
      level = self.level,
      parent = self,
      void_rift = self.parent.void_rift,
    ...
    if self.level == 3 then
      self.parent.t:every(0.3, function()
        ...
        Area({
          group = main.current.effects,
          x = self.x + random:float(-32, 32),
          y = self.y + random:float(-32, 32),
          r = self.r + random:float(0, 2 * math.pi),
          w = self.parent.area_size_m * 48,
          color = self.color,
          dmg = 0.5 * self.parent.area_dmg_m * self.dmg,
          character = self.character,
          level = self.level,
          parent = self,
          void_rift = self.parent.void_rift,
...
function Projectile:on_collision_enter(other, contact)
  ...
  if other:is(Wall) then
    if
      self.character == "archer"
      or self.character == "hunter"
      or self.character == "barrage"
      or self.character == "barrager"
      or self.character == "sentry"
    then
      ...
      _G[random:table({ "arrow_hit_wall1", "arrow_hit_wall2" })]:play({
        pitch = random:float(0.9, 1.1),
        volume = 0.2,
      })
    elseif
    ...
    elseif
      ...
      or self.character == "lich"
      or self.character == "arcanist"
      or self.character == "arcanist_projectile"
      or self.character == "witch"
    then
      self:die(x, y, r, random:int(2, 3))
      magic_area1:play({ pitch = random:float(0.95, 1.05), volume = 0.075 })
    ...
    elseif
      self.character == "engineer"
      or self.character == "dual_gunner"
      or self.character == "miner"
    then
      self:die(x, y, r, random:int(2, 3))
      _G[random:table({ "turret_hit_wall1", "turret_hit_wall2" })]:play({
        pitch = random:float(0.9, 1.1),
        volume = 0.2,
      })
    elseif self.character == "psyker" then
  ...
  if table.any(main.current.enemies, function(v)
    ...
    if self.pierce <= 0 and self.chain <= 0 then
      ...
      if self.chain > 0 then
        ...
        local object = self:get_random_object_in_shape(
          Circle(self.x, self.y, 48),
          main.current.enemies,
          self.chain_enemies_hit
        )
        if object then
          ...
          if self.parent.ultimatum then
            self.dmg = self.dmg
              * (
                (self.parent.ultimatum == 1 and 1.1)
                or (self.parent.ultimatum == 2 and 1.2)
                or (self.parent.ultimatum == 3 and 1.3)
              )
          end
        end
      end
    ...
    if
      self.character == "archer"
      or self.character == "scout"
      or self.character == "outlaw"
      or self.character == "blade"
      or self.character == "hunter"
      or self.character == "spellblade"
      or self.character == "engineer"
      or self.character == "jester"
      or self.character == "assassin"
      or self.character == "barrager"
      or self.character == "beastmaster"
        ...
        if self.level == 3 then
          ...
          SpawnEffect({
            ...
            action = function(x, y)
              ...
              Pet({
                group = main.current.main,
                x = x + 12 * math.cos(r + math.pi / 2),
                y = y + 12 * math.sin(r + math.pi / 2),
                r = r,
                v = 150,
                parent = self.parent,
                conjurer_buff_m = self.conjurer_buff_m or 1,
          ...
          SpawnEffect({
            ...
            action = function(x, y)
              Pet({
                ...
                x = x,
                y = y,
                r = self.parent:angle_to_object(other),
                v = 150,
                parent = self.parent,
                conjurer_buff_m = self.conjurer_buff_m or 1,
              })
    ...
    if self.character == "assassin" then
      other:apply_dot(
        (self.crit and 4 * self.dmg or self.dmg / 2)
          * (self.dot_dmg_m or 1)
          * (main.current.chronomancer_dot or 1),
        3
      )
    end
    ...
    if self.parent and self.parent.chain_infused then
      ...
      for i = 1, 2 + (stormweaver_level == 3 and 2 or 0) do
        ...
        local dst = src:get_random_object_in_shape(
          Circle(src.x, src.y, (stormweaver_level == 3 and 128 or 64)),
          main.current.enemies,
          self.infused_enemies_hit
        )
        if dst then
    ...
    if self.parent and self.parent.lightning_strike then
      if
        random:bool(
          (self.parent.lightning_strike == 1 and 5)
            or (self.parent.lightning_strike == 2 and 10)
            or (self.parent.lightning_strike == 3 and 15)
        )
      then
        local src = other
        for j = 1, 3 do
          main.current.t:after((j - 1) * 0.1, function()
            ...
            for i = 1, 3 do
              ...
              if dst then
                dst:hit(
                  0.33
                    * ((self.parent.lightning_strike == 1 and 0.6) or (self.parent.lightning_strike == 2 and 0.8) or (self.parent.lightning_strike == 3 and 1))
                    * self.dmg
                    * (self.distance_dmg_m or 1)
                )
                LightningLine({ group = main.current.effects, src = src, dst = dst })
                src = dst
              end
    ...
    if self.crit then
      ...
      HitCircle({
        ...
        color = fg[0],
        duration = 0.3,
      }):scale_down():change_color(0.5, self.color)
    end
    ...
    if
      self.parent
      and self.parent.explosive_arrow
      and table.any(self.parent.classes, function(v)
        return v == "ranger"
      end)
    then
      if
        random:bool(
          (self.parent.explosive_arrow == 1 and 10)
            or (self.parent.explosive_arrow == 2 and 20)
            or (self.parent.explosive_arrow == 3 and 30)
        )
      then
        _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
          pitch = random:float(0.95, 1.05),
        ...
        Area({
          ...
          color = self.color,
          dmg = (
            (self.parent.explosive_arrow == 1 and 0.1)
            or (self.parent.explosive_arrow == 2 and 0.2)
            or (self.parent.explosive_arrow == 3 and 0.3)
          )
            * self.parent.area_dmg_m
            * self.dmg,
          character = self.character,
          level = self.level,
          parent = self,
    ...
    if
      self.parent
      and self.parent.void_rift
      and table.any(self.parent.classes, function(v)
        return v == "mage" or v == "nuker" or v == "voider"
      end)
    then
      if random:bool(20) then
        DotArea({
          ...
          x = self.x,
          y = self.y,
          rs = self.parent.area_size_m * 24,
          color = self.color,
          dmg = self.parent.area_dmg_m * self.dmg * (self.parent.dot_dmg_m or 1),
          void_rift = true,
          duration = 1,
        })
      end
...
function Area:init(args)
  ...
  for _, enemy in ipairs(enemies) do
    local resonance_dmg = 0
    local resonance_m = (self.parent.resonance == 1 and 0.03)
      or (self.parent.resonance == 2 and 0.05)
      or (self.parent.resonance == 3 and 0.07)
      or 0
    if self.character == "elementor" then
      if self.parent.resonance then
        resonance_dmg = 2 * self.dmg * resonance_m * #enemies
      end
    ...
    elseif self.character == "swordsman" then
      if self.parent.resonance then
        resonance_dmg = (self.dmg + self.dmg * 0.15 * #enemies) * resonance_m * #enemies
      end
      enemy:hit(self.dmg + self.dmg * 0.15 * #enemies + resonance_dmg, self)
    elseif self.character == "blade" and self.level == 3 then
      if self.parent.resonance then
        resonance_dmg = (self.dmg + self.dmg * 0.33 * #enemies) * resonance_m * #enemies
      end
      enemy:hit(self.dmg + self.dmg * 0.33 * #enemies + resonance_dmg, self)
    elseif self.character == "highlander" then
      if self.parent.resonance then
        resonance_dmg = 6 * self.dmg * resonance_m * #enemies
      end
    ...
    elseif self.character == "freezing_field" then
      ...
      if self.parent.resonance then
        resonance_dmg = self.dmg * resonance_m * #enemies
      end
    ...
    if
      self.character == "wizard"
      or self.character == "magician"
      or self.character == "elementor"
      or self.character == "psychic"
    then
    ...
    elseif
      ...
      or self.character == "pyromancer"
      or self.character == "bomber"
    then
      if self.character == "pyromancer" then
        pyro2:play({ pitch = random:float(0.95, 1.05), volume = 0.4 })
      end
      _G[random:table({ "saboteur_hit1", "saboteur_hit2" })]:play({
        pitch = random:float(0.95, 1.05),
        volume = 0.2,
    ...
    if self.stun then
      ...
      enemy.t:after(self.stun, function()
        enemy.barbarian_stunned = false
      end)
    end
  ...
  if self.parent:is(Projectile) then
    ...
    if
      p.void_rift
      and table.any(p.classes, function(v)
        return v == "mage" or v == "nuker" or v == "voider"
      end)
    then
      if random:bool(20) then
        DotArea({
          ...
          rs = p.area_size_m * 24,
          color = self.color,
          dmg = p.area_dmg_m * self.dmg * (p.dot_dmg_m or 1),
          void_rift = true,
          duration = 1,
          parent = p,
        })
    ...
    if p.echo_barrage and not self.echo_barrage_area then
      if
        random:bool(
          (p.echo_barrage == 1 and 10)
            or (p.echo_barrage == 2 and 20)
            or (p.echo_barrage == 3 and 30)
        )
      then
        p.t:every(0.3, function()
          ...
          Area({
            ...
            w = p.area_size_m * 48,
            color = p.color,
            dmg = 0.5 * p.area_dmg_m * self.dmg,
            character = self.character,
            level = p.level,
            parent = p,
            echo_barrage_area = true,
          })
        end, p.echo_barrage)
      end
    end
    ...
    if self.parent.echo_barrage and not self.echo_barrage_area then
      if
        random:bool(
          (self.parent.echo_barrage == 1 and 10)
            or (self.parent.echo_barrage == 2 and 20)
            or (self.parent.echo_barrage == 3 and 30)
        )
      then
        self.parent.t:every(0.3, function()
          _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
          ...
          Area({
            ...
            w = self.parent.area_size_m * 48,
            color = self.parent.color,
            dmg = 0.5 * self.parent.area_dmg_m * (self.dmg or self.parent.dmg),
            character = self.character,
            level = self.parent.level,
            parent = self.parent,
            echo_barrage_area = true,
  ...
  self.w = 0
  self.hidden = false
  self.t:tween(0.05, self, { w = args.w }, math.cubic_in_out, function()
    self.spring:pull(0.15)
  end)
...
function DotArea:init(args)
  ...
  if
    self.character == "plague_doctor"
    or self.character == "pyromancer"
    or self.character == "witch"
    or self.character == "burning_field"
  then
  ...
  if self.character == "witch" then
    ...
    if self.level == 3 then
      self.t:every(1, function()
        local enemies =
          main.current.main:get_objects_in_shape(self.closest_sensor, main.current.enemies)
        if enemies and #enemies > 0 then
          ...
          Projectile(table.merge(t, mods or {}))
          _G[random:table({ "scout1", "scout2" })]:play({
            pitch = random:float(0.95, 1.05),
            volume = 0.35,
          })
          wizard1:play({ pitch = random:float(0.95, 1.05), volume = 0.15 })
  ...
  if self.void_rift then
    self.dvr = random:table({
      random:float(-4 * math.pi, -2 * math.pi),
      random:float(2 * math.pi, 4 * math.pi),
    })
  end
end
...
function DotArea:update(dt)
  ...
  if self.parent then
    if
      (self.character == "plague_doctor" and self.level == 3 and not self.plague_doctor_unmovable)
      or self.character == "cryomancer"
      or self.character == "pyromancer"
    then
  ...
  if self.character == "witch" then
    self.x, self.y =
      self.x + self.v * math.cos(self.r) * dt, self.y + self.v * math.sin(self.r) * dt
    if
      self.x >= main.current.x2 - self.shape.rs / 2 or self.x <= main.current.x1 + self.shape.rs / 2
    then
      self.r = math.pi - self.r
    end
    if
      self.y >= main.current.y2 - self.shape.rs / 2 or self.y <= main.current.y1 + self.shape.rs / 2
    then
...
function ForceArea:init(args)
  ...
  self.vr = 0
  self.dvr = random:table({
    random:float(-6 * math.pi, -4 * math.pi),
    random:float(4 * math.pi, 6 * math.pi),
  })
...
function Tree:init(args)
  ...
  self.t:every(self.parent.level == 3 and 3 or 6, function()
    ...
    if
      self.parent.taunt
      and random:bool(
        (self.parent.taunt == 1 and 10)
          or (self.parent.taunt == 2 and 20)
          or (self.parent.taunt == 3 and 30)
      )
    then
      local enemies = self:get_objects_in_shape(Circle(self.x, self.y, 96), main.current.enemies)
    ...
    if self.parent.rearm then
      self.t:after(0.25, function()
        ...
        then
          local enemies =
            self:get_objects_in_shape(Circle(self.x, self.y, 96), main.current.enemies)
          if #enemies > 0 then
            for _, enemy in ipairs(enemies) do
              enemy.taunted = self
              enemy.t:after(4, function()
    ...
    self.t:every_immediate(
      ...
      function()
        ...
        if self.parent.construct_instability then
          camera:shake(2, 0.5)
          local n = (self.parent.construct_instability == 1 and 1)
            or (self.parent.construct_instability == 2 and 1.5)
            or (self.parent.construct_instability == 3 and 2)
            or 1
          Area({
            group = main.current.effects,
            x = self.x,
            ...
            w = self.parent.area_size_m * 48,
            color = self.color,
            dmg = n * self.parent.dmg * self.parent.area_dmg_m,
            parent = self.parent,
          })
          _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
            pitch = random:float(0.95, 1.05),
...
function ForceField:init(args)
  ...
  self:set_as_circle(
    (
      self.parent and self.parent.magnify and (self.parent.magnify == 1 and 14)
      or (self.parent.magnify == 2 and 17)
      or (self.parent.magnify == 3 and 20)
    ) or 12,
    "static",
    "force_field"
  )
...
function ForceField:draw()
  ...
  graphics.circle(
    ...
    self.y,
    self.shape.rs,
    self.hfx.hit.f and fg_transparent[0] or self.color_transparent
  )
  graphics.pop()
end
...
function ForceField:on_collision_enter(other, contact)
  ...
  if table.any(main.current.enemies, function(v)
    ...
    other:push(
      random:float(15, 20) * (self.parent.knockback_m or 1),
      self.parent:angle_to_object(other)
    )
    other:hit(0)
    HitCircle({ group = main.current.effects, x = x, y = y, rs = 6, color = fg[0], duration = 0.1 })
    for i = 1, 2 do
...
function Volcano:init(args)
  ...
  self.t:every(self.level == 3 and 0.5 or 1, function()
    camera:shake(4, 0.5)
    _G[random:table({ "earth1", "earth2", "earth3" })]:play({
      pitch = random:float(0.95, 1.05),
      volume = 0.25,
    })
    _G[random:table({ "fire1", "fire2", "fire3" })]:play({
      pitch = random:float(0.95, 1.05),
      volume = 0.25,
    })
    Area({
      ...
      y = self.y,
      r = self.r,
      w = self.parent.area_size_m * 72,
      r = random:float(0, 2 * math.pi),
      color = self.color,
      dmg = (self.parent.area_dmg_m or 1) * self.parent.dmg,
...
function Sentry:init(args)
  ...
  self.t:every({ 2.75, 3.5 }, function()
    ...
    local r = self.r
    local n = random:bool(
      (main.current.ranger_level == 2 and 16) or (main.current.ranger_level == 1 and 8) or 0
    ) and 4 or 1
    for j = 1, n do
      self.t:after((j - 1) * 0.1, function()
        for i = 1, 4 do
          ...
          local t = {
            ...
            r = r,
            color = self.color,
            dmg = self.parent.dmg * (self.parent.conjurer_buff_m or 1),
            character = "sentry",
            parent = self.parent,
            ricochet = self.parent.level == 3 and 2 or 0,
          }
          Projectile(table.merge(t, mods or {}))
          r = r + math.pi / 2
        end
...
function Turret:init(args)
  ...
  self.t:every({ 2.75, 3.5 }, function()
    self.t:every({ 0.1, 0.2 }, function()
      ...
      HitCircle({
        group = main.current.effects,
        x = self.x + 0.8 * self.shape.w * math.cos(self.r),
        y = self.y + 0.8 * self.shape.w * math.sin(self.r),
        rs = 6,
      })
      local t = {
        group = main.current.main,
        x = self.x + 1.6 * self.shape.w * math.cos(self.r),
        y = self.y + 1.6 * self.shape.w * math.sin(self.r),
        v = 200,
        r = self.r,
        color = self.color,
        dmg = self.parent.dmg * (self.parent.conjurer_buff_m or 1) * self.upgrade_dmg_m,
        character = self.parent.character,
        parent = self.parent,
      }
      Projectile(table.merge(t, mods or {}))
...
function Bomb:explode()
  ...
  local t = {
    ...
    y = self.y,
    r = self.r,
    w = self.parent.area_size_m * 64 * (self.level == 3 and 2 or 1),
    color = self.color,
    dmg = self.parent.area_dmg_m
      * self.dmg
      * (self.parent.conjurer_buff_m or 1)
      * (self.level == 3 and 2 or 1),
    character = self.character,
    parent = self.parent,
  }
  Area(table.merge(t, mods or {}))
  if not self.parent.construct_instability and not self.parent.rearm then
    self.dead = true
  end
  _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
    pitch = random:float(0.95, 1.05),
  ...
  self.t:after(0.25, function()
    if self.parent.construct_instability then
      ...
      Area({
        ...
        x = self.x,
        y = self.y,
        r = self.r + random:float(-math.pi / 16, math.pi / 16),
        w = self.parent.area_size_m * 48 * (self.level == 3 and 2 or 1),
        color = self.color,
        dmg = n * self.parent.dmg * self.parent.area_dmg_m * (self.level == 3 and 2 or 1),
        parent = self.parent,
      })
      _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
        pitch = random:float(0.95, 1.05),
...
function Saboteur:init(args)
  ...
  _G[random:table({ "saboteur1", "saboteur2", "saboteur3" })]:play({
    pitch = random:float(0.8, 1.2),
    volume = 0.2,
  })
...
function Saboteur:on_collision_enter(other, contact)
  if table.any(main.current.enemies, function(v)
    ...
    local t = {
      ...
      y = self.y,
      r = self.r,
      w = (self.crit and 1.5 or 1) * self.area_size_m * 64,
      color = self.color,
      dmg = (self.crit and 2 or 1)
        * self.area_dmg_m
        * self.actual_dmg
        * (self.conjurer_buff_m or 1),
      character = self.character,
      parent = self.parent,
    }
    Area(table.merge(t, mods or {}))
...
function Gold:init(args)
  ...
  if #self.group:get_objects_by_class(Gold) > 30 then
    self.dead = true
    return
  end
  ...
  self:apply_angular_impulse(
    random:table({ random:float(-6 * math.pi, -2 * math.pi), random:float(2 * math.pi, 6 * math.pi) })
  )
...
function Gold:update(dt)
  self:update_game_object(dt)
  self.r = self:get_angle()
  if not self.magnet_sensor then
    return
  end
  if not self.weak_magnet_sensor then
    return
  end

  local players = self:get_objects_in_shape(
    main.current.player.magnetism and self.magnet_sensor or self.weak_magnet_sensor,
    { Player }
  )
  ...
  if self.magnet_sensor then
    self.magnet_sensor:move_to(self.x, self.y)
  end
  if self.weak_magnet_sensor then
    self.weak_magnet_sensor:move_to(self.x, self.y)
  end
end
...
function Gold:on_trigger_enter(other, contact)
  if self.cant_be_picked_up then
    return
  end
  ...
  if other:is(Player) then
    ...
    _G[random:table({ "gold2", "coins1", "coins2", "coins3" })]:play({
      pitch = random:float(0.9, 1.1),
      volume = 0.3,
    })
    ...
    if th then
      if th.level == 3 then
        trigger:after(0.01, function()
          if not main.current.main.world then
            return
          end
          _G[random:table({ "scout1", "scout2" })]:play({
            pitch = random:float(0.95, 1.05),
...
function HealingOrb:init(args)
  ...
  if #self.group:get_objects_by_class(HealingOrb) > 30 then
    self.dead = true
    return
  end
  ...
  if main.current.healer_level > 0 and not self.healer_effect_orb then
    if
      random:bool(
        (main.current.healer_level == 1 and 15) or (main.current.healer_level == 2 and 30)
      )
    then
      SpawnEffect({
        group = main.current.effects,
...
function HealingOrb:draw()
  ...
  graphics.circle(
    ...
    self.y,
    1.2 * self.shape.w,
    self.hfx.hit.f and fg[0] or green_transparent_weak
  )
...
function HealingOrb:on_trigger_enter(other, contact)
  ...
  if other:is(Player) then
    ...
    for i = 1, 2 do
      HitParticle({ group = main.current.effects, x = self.x, y = self.y, color = green[0] })
    end
    ...
    if
      main.current.player.divine_barrage
      and random:bool(
        (main.current.player.divine_barrage == 1 and 20)
          or (main.current.player.divine_barrage == 2 and 40)
          or (main.current.player.divine_barrage == 3 and 60)
      )
    then
      trigger:after(0.01, function()
        if not main.current.main.world then
...
function Critter:init(args)
  ...
  if #self.group:get_objects_by_class(Critter) > 100 then
    self.dead = true
    return
  end
  self:init_unit()
  ...
  self.hp = 1
    + ((main.current.swarmer_level == 2 and 3) or (main.current.swarmer_level == 1 and 1) or 0)
    + (self.parent.hive or 0)
end
...
function Critter:hit(damage)
  if self.dead or self.invulnerable then
    return
  end
  self.hfx:use("hit", 0.25, 200, 10)
  self.hp = self.hp - 1
...
function Critter:push(f, r)
  ...
  self:apply_impulse(f * math.cos(r), f * math.sin(r))
  self:apply_angular_impulse(
    random:table({ random:float(-12 * math.pi, -4 * math.pi), random:float(
      4 * math.pi,
      12 * math.pi
    ) })
  )
...
function Critter:die(x, y, r, n)
  ...
  HitCircle({ group = main.current.effects, x = x, y = y }):scale_down()
  self.dead = true
  _G[random:table({ "enemy_die1", "enemy_die2" })]:play({
    pitch = random:float(0.9, 1.1),
    volume = 0.5,
  })
  ...
  if main.current.player.baneling_burst then
    ...
    Area({
      ...
      w = self.parent.area_size_m * 24,
      color = self.color,
      dmg = (main.current.player.baneling_burst == 1 and 50)
        or (main.current.player.baneling_burst == 2 and 100)
        or (main.current.player.baneling_burst == 3 and 150)
        or 0,
      parent = self.parent,
    })
    _G[random:table({ "cannoneer1", "cannoneer2" })]:play({
