...
function BuyScreen:on_enter(from, level, loop, units, passives, shop_level, shop_xp)
  ...
  self.items_text = Text(
    { { text = "[wavy_mid, fg]items", font = pixul_font, alignment = "center" } },
    global_text_tags
  )
  self.ng_text = Text(
    { { text = "[fg]NG+" .. current_new_game_plus, font = pixul_font, alignment = "center" } },
    global_text_tags
  )
  local get_elite_str = function(lvl)
    if (lvl - (25 * self.loop)) % 6 == 0 or lvl % 25 == 0 then
      return " (elite)"
    elseif (lvl - (25 * self.loop)) % 3 == 0 then
      return " (hard)"
    else
      return ""
    end
  ...
  self.tutorial_button = Button({
    ...
    action = function()
      ...
      self.title_text = Text2({
        ...
        x = gw / 2,
        y = 35,
        lines = { { text = "[fg]WELCOME TO SNKRX!", font = fat_font, alignment = "center" } },
      })
      self.tutorial_text = Text2({
        ...
        lines = {
          {
            text = "[fg]You control a snake of multiple heroes that auto-attack nearby enemies.",
            font = pixul_font,
            height_multiplier = 1.2,
          },
          {
            text = "[fg]You can steer the snake left or right by pressing [yellow]A/D[fg] or [yellow]left/right arrows[fg].",
            font = pixul_font,
            height_multiplier = 2.2,
          },
          {
            text = "[fg]Combine the same heroes to level them up:",
            font = pixul_font,
            height_multiplier = 1.2,
          },
          {
            text = "[fg]At [yellow]Lv.3[fg] heroes unlock special effects.",
            font = pixul_font,
            height_multiplier = 2.2,
          },
          {
            text = "[fg]Hire heroes of the same classes to unlock class passives:",
            font = pixul_font,
            height_multiplier = 1.2,
          },
          {
            text = "[fg]Each hero can have between [yellow]1 to 3[fg] classes.",
            font = pixul_font,
            height_multiplier = 2.2,
          },
          {
            text = "[fg]You gain [yellow]1 interest per 5 gold[fg], up to a maximum of 5.",
            font = pixul_font,
            height_multiplier = 1.2,
          },
          {
            text = "[fg]This means that saving above [yellow]25 gold[fg] doesn't yield more interest.",
            font = pixul_font,
            height_multiplier = 2.2,
          },
          {
            text = "[yellow, wavy_mid]Good luck!",
            font = pixul_font,
            height_multiplier = 2.2,
            alignment = "center",
          },
  ...
  locked_state = {
    locked = self.locked,
    cards = {
      self.cards[1] and self.cards[1].unit,
      self.cards[2] and self.cards[2].unit,
      self.cards[3] and self.cards[3].unit,
    },
  }
  system.save_run(
    self.level,
...
function BuyScreen:update(dt)
  if main_song_instance:isStopped() then
    main_song_instance =
      _G[random:table({ "song1", "song2", "song3", "song4", "song5" })]:play({ volume = 0.2 })
  end
  ...
  if not self.in_tutorial and not self.paused then
    ...
    if self.sets_text then
      self.sets_text:update(dt)
    end
    if self.party_text then
      self.party_text:update(dt)
    end
    if self.items_text then
      self.items_text:update(dt)
    end
    if self.ng_text then
      self.ng_text:update(dt)
    end
    if self.level_text then
      self.level_text:update(dt)
    end
  else
    self.ui:update(dt * slow_amount)
...
function BuyScreen:quit_tutorial()
  ...
  self.title_text = nil
  for _, t in ipairs(self.tutorial_cards) do
    t.dead = true
  end
...
function BuyScreen:draw()
  ...
  self.effects:draw()
  if self.items_text then
    self.items_text:draw(32, 145)
  end
  if self.level_text then
    self.level_text:draw(265, gh - 20)
  end
  ...
  if self.unit_grabbed then
    ...
    graphics.print_centered(
      self.unit_grabbed.level,
      pixul_font,
      self.unit_grabbed.x + 0.5,
      y + 2,
      0,
      1,
      1,
  ...
  if self.shop_text then
    self.shop_text:draw(64, 20)
  end
  if self.sets_text then
    self.sets_text:draw(328, 20)
  end
  if self.party_text then
    self.party_text:draw(440, 20)
  end
  if current_new_game_plus > 0 then
    self.ng_text:draw(265, gh - 40)
  end
...
function BuyScreen:buy(character, i)
  local bought
  if
    table.any(self.units, function(v)
      return v.character == character
    end) and gold >= character_tiers[character]
  then
    if
      table.any(self.units, function(v)
        return v.character == character and v.level == 3
      end)
    then
      if not self.info_text then
        ...
        self.info_text:activate({
          {
            text = "[fg]this unit has already reached max level",
            font = pixul_font,
            alignment = "center",
          },
        }, nil, nil, nil, nil, 16, 4, nil, 2)
      ...
      self.shop_text:set_text({
        {
          text = "[wavy_mid, fg]shop [fg]- [fg, nudge_down]gold: [yellow, nudge_down]" .. gold,
          font = pixul_font,
          alignment = "center",
        },
      })
    ...
    if #self.units >= max_units then
      if not self.info_text then
        ...
        self.info_text:activate({
          {
            text = "[fg]maximum number of units [yellow](" .. max_units .. ") [fg]reached",
            font = pixul_font,
            alignment = "center",
          },
        }, nil, nil, nil, nil, 16, 4, nil, 2)
...
function BuyScreen:set_cards(shop_level, dont_spawn_effect, first_call)
  if self.cards then
    for i = 1, 3 do
      if self.cards[i] then
        self.cards[i]:die(dont_spawn_effect)
      end
    end
  end
  self.cards = {}
  ...
  until not table.all(all_units, function(v)
      return table.any(non_attacking_characters, function(u)
        return v == u
      end)
    end)
  if first_call and locked_state then
    if locked_state.cards[1] then
      self.cards[1] = ShopCard({
        group = self.main,
        x = 60,
        y = 75,
        w = 80,
        h = 90,
        unit = locked_state.cards[1],
        parent = self,
        i = 1,
      })
    end
    if locked_state.cards[2] then
      self.cards[2] = ShopCard({
        group = self.main,
        x = 140,
        y = 75,
        w = 80,
        h = 90,
        unit = locked_state.cards[2],
        parent = self,
        i = 2,
      })
    end
    if locked_state.cards[3] then
      self.cards[3] = ShopCard({
        group = self.main,
        x = 220,
        y = 75,
        w = 80,
        h = 90,
        unit = locked_state.cards[3],
        parent = self,
        i = 3,
      })
    end
...
function BuyScreen:set_party_and_sets()
  ...
  for i, unit in ipairs(self.units) do
    table.insert(
      ...
      CharacterPart({
        ...
        i = i,
        spawn_effect = unit.spawn_effect,
        parent = self,
      })
    )
  ...
  if self.sets then
    for _, icon in ipairs(self.sets) do
      icon:die(true)
    end
  end
  self.sets = {}
  ...
  for i, class in ipairs(classes) do
    local x, y
    if #classes <= 6 then
      x, y = math.index_to_coordinates(i, 2)
    else
      x, y = math.index_to_coordinates(i, 3)
    end
    table.insert(
      self.sets,
      ClassIcon({
        group = self.main,
        x = (#classes <= 6 and 319 or 308) + (x - 1) * 20,
        y = 45 + (y - 1) * 56,
        class = class,
        units = self.units,
        parent = self,
...
function SteamFollowButton:init(args)
  ...
  self.shape = Rectangle(
    self.x,
    self.y,
    pixul_font:get_text_width("follow me on steam!") + 12,
    pixul_font.h + 4
  )
  self.text = Text(
    { { text = "[greenm5]follow me on steam!", font = pixul_font, alignment = "center" } },
    global_text_tags
  )
end
...
function SteamFollowButton:update(dt)
  ...
  if main.current.in_credits then
    return
  end
...
function SteamFollowButton:draw()
  ...
  graphics.rectangle(
    ...
    4,
    4,
    self.selected and fg[0] or green[0]
  )
...
function SteamFollowButton:on_mouse_enter()
  ...
  self.selected = true
  self.text:set_text({
    { text = "[fgm5]follow me on steam!", font = pixul_font, alignment = "center" },
  })
...
function WishlistButton:init(args)
  ...
  if self.w_to_wishlist then
    ...
    self.text = Text(
      { { text = "[bg10]w to wishlist", font = pixul_font, alignment = "center" } },
      global_text_tags
    )
  else
    self.shape = Rectangle(self.x, self.y, 110, 18)
    self.text = Text(
      { { text = "[bg10]wishlist on steam", font = pixul_font, alignment = "center" } },
      global_text_tags
    )
  end
end
...
function WishlistButton:draw()
  ...
  graphics.rectangle(
    ...
    4,
    4,
    self.selected and fg[0] or bg[1]
  )
...
function WishlistButton:on_mouse_enter()
  ...
  if self.w_to_wishlist then
    ...
    self.text:set_text({
      { text = "[fgm5]wishlist on steam", font = pixul_font, alignment = "center" },
    })
  end
  self.spring:pull(0.2, 200, 10)
end
...
function RestartButton:on_mouse_enter()
  ...
  self.text:set_text({
    {
      text = "[fgm5]NG+" .. tostring(current_new_game_plus),
      font = pixul_font,
      alignment = "center",
    },
  })
...
function Button:on_mouse_enter()
  ...
  self.selected = true
  self.text:set_text({
    { text = "[fgm5]" .. self.button_text, font = pixul_font, alignment = "center" },
  })
  self.spring:pull(0.2, 200, 10)
...
function GoButton:init(args)
  ...
  self.interact_with_mouse = true
  self.text =
    Text({ { text = "[greenm5]GO!", font = pixul_font, alignment = "center" } }, global_text_tags)
end
  ...
  if ((self.selected and input.m1.pressed) or input.enter.pressed) and not self.transitioning then
    if #self.parent.units == 0 then
      if not self.info_text then
        ...
        self.info_text:activate({
          {
            text = "[fg]cannot start the round with [yellow]0 [fg]units",
            font = pixul_font,
            alignment = "center",
          },
        }, nil, nil, nil, nil, 16, 4, nil, 2)
      ...
      locked_state = {
        locked = self.parent.locked,
        cards = {
          self.parent.cards[1] and self.parent.cards[1].unit,
          self.parent.cards[2] and self.parent.cards[2].unit,
          self.parent.cards[3] and self.parent.cards[3].unit,
        },
      }
      ui_switch2:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
      self.spring:pull(0.2, 200, 10)
      ...
      TransitionEffect({
        ...
        x = self.x,
        y = self.y,
        color = state.dark_transitions and bg[-2]
          or character_colors[random:table(self.parent.units).character],
        transition_action = function()
...
function LockButton:update(dt)
  ...
  if self.selected and input.m1.pressed then
    ...
    if not self.parent.locked then
      locked_state = nil
    end
    if self.parent.locked then
      locked_state = {
...
function LockButton:draw()
  ...
  graphics.rectangle(
    ...
    4,
    4,
    (self.selected or self.parent.locked) and fg[0] or bg[1]
  )
  self.text:draw(self.x, self.y + 1)
  graphics.pop()
end
...
function LevelButton:init(args)
  ...
  self.max_xp = (self.parent.shop_level == 1 and 3)
    ...
    or (self.parent.shop_level == 3 and 5)
    or (self.parent.shop_level == 4 and 6)
    or (self.parent.shop_level == 5 and 0)
end
...
function LevelButton:update(dt)
  ...
  if self.selected and input.m1.pressed then
    ...
    if gold < 5 then
      ...
      self.t:after(2, function()
        self.info_text_2:deactivate()
        self.info_text_2.dead = true
        self.info_text_2 = nil
      end, "info_text_2")
    else
      ui_switch2:play({ pitch = random:float(0.95, 1.05), volume = 0.5 })
      ...
      self.spring:pull(0.2, 200, 10)
      gold = gold - 5
      self.parent.shop_text:set_text({
        {
          text = "[wavy_mid, fg]shop [fg]- [fg, nudge_down]gold: [yellow, nudge_down]" .. gold,
          font = pixul_font,
          alignment = "center",
  ...
  if self.selected and input.m2.pressed then
    if self.parent.shop_level <= 1 then
      return
    end
    if gold < 10 then
      self.spring:pull(0.2, 200, 10)
...
function LevelButton:draw()
  ...
  for i = 1, self.max_xp do
    graphics.line(
      self.x + 0.9 * self.shape.w + (i - 1) * 5,
      self.y - self.shape.h / 3,
      self.x + 0.9 * self.shape.w + (i - 1) * 5,
      self.y + self.shape.h / 3,
      bg[1],
      2
...
function LevelButton:create_info_text()
  ...
  if self.parent.shop_level < 5 then
    local t11, t12 =
      get_shop_odds(self.parent.shop_level, 1), get_shop_odds(self.parent.shop_level + 1, 1)
    local t21, t22 =
      get_shop_odds(self.parent.shop_level, 2), get_shop_odds(self.parent.shop_level + 1, 2)
    local t31, t32 =
      get_shop_odds(self.parent.shop_level, 3), get_shop_odds(self.parent.shop_level + 1, 3)
    local t41, t42 =
      get_shop_odds(self.parent.shop_level, 4), get_shop_odds(self.parent.shop_level + 1, 4)
    self.info_text = InfoText({ group = main.current.ui })
    self.info_text:activate({
      {
        text = "[yellow]Lv."
          ...
          .. "/"
          .. self.max_xp
          .. "[fg], +1 XP cost: [yellow]5",
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.5,
      },
      {
        text = "[bg10]chances of units appearing on the shop",
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25,
      },
      {
        text = "[yellow]current shop level                  [fgm10]next shop level",
        font = pixul_font,
        alignment = "left",
        height_multiplier = 1.25,
      },
  ...
  elseif self.parent.shop_level == 5 then
    ...
    self.info_text:activate({
      {
        text = "[yellow]Lv." .. self.parent.shop_level .. "[fg] shop",
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.5,
      },
      ...
      {
        text = "[yellow]current shop level",
        font = pixul_font,
        alignment = "left",
        height_multiplier = 1.25,
      },
      ...
      {
        text = "[green]tier 2: " .. t21 .. "%",
        font = pixul_font,
        alignment = "left",
        height_multiplier = 1.25,
      },
      {
        text = "[blue]tier 3: " .. t31 .. "%",
        font = pixul_font,
        alignment = "left",
        height_multiplier = 1.25,
      },
      {
        text = "[purple]tier 4: " .. t41 .. "%",
        font = pixul_font,
        alignment = "left",
        height_multiplier = 1.25,
      },
...
function RerollButton:init(args)
  ...
  if self.parent:is(BuyScreen) then
    ...
    self.text = Text(
      { { text = "[bg10]reroll: [yellow]2", font = pixul_font, alignment = "center" } },
      global_text_tags
    )
  elseif self.parent:is(Arena) then
    ...
    if self.parent.level == 3 or (merchant and merchant.level == 3) then
      ...
      self.text = Text(
        { { text = "[bg10]reroll: [yellow]0", font = pixul_font, alignment = "center" } },
        global_text_tags
      )
    else
      self.text = Text(
        { { text = "[bg10]reroll: [yellow]5", font = pixul_font, alignment = "center" } },
        global_text_tags
      )
    end
  end
  ...
  if (self.selected and input.m1.pressed) or input.r.pressed then
    ...
    elseif self.parent:is(Arena) then
      if gold < 5 and not self.free_reroll then
        ...
        self.selected = true
        self.spring:pull(0.2, 200, 10)
        if not self.free_reroll then
          gold = gold - 5
        end
        self.parent.shop_text:set_text({
          {
            text = "[fg, nudge_down]gold: [yellow, nudge_down]" .. gold,
            font = pixul_font,
            alignment = "center",
          },
        })
...
function RerollButton:draw()
  ...
  if self.parent:is(Arena) then
    graphics.rectangle(
      ...
      4,
      4,
      self.selected and fg[0] or bg[-2]
    )
  else
    graphics.rectangle(
      self.x,
...
function RerollButton:on_mouse_exit()
  ...
  elseif self.parent:is(Arena) then
    if self.free_reroll then
      self.text:set_text({
        { text = "[fgm5]reroll: [yellow]0", font = pixul_font, alignment = "center" },
      })
    else
      self.text:set_text({
        { text = "[fgm5]reroll: [yellow]5", font = pixul_font, alignment = "center" },
      })
    end
  end
  self.selected = false
...
function TutorialCharacterPart:draw()
  ...
  graphics.rectangle(
    ...
    3,
    3,
    self.highlighted and fg[0] or character_colors[self.character]
  )
  graphics.print_centered(
    self.level,
    pixul_font,
    ...
    0,
    0,
    self.highlighted and fg[-5] or _G[character_color_strings[self.character]][-5]
  )
  graphics.pop()
end
...
function TutorialCharacterPart:on_mouse_enter()
  ...
  self.info_text:activate({
    {
      text = "["
        .. character_color_strings[self.character]
        .. "]"
        .. self.character:capitalize()
        .. "[fg] - [yellow]Lv."
        .. self.level,
      font = pixul_font,
      alignment = "center",
    ...
    {
      text = character_descriptions[self.character](self.level),
      font = pixul_font,
      alignment = "center",
      height_multiplier = 2,
    },
    {
      text = "["
        .. (self.level == 3 and "yellow" or "light_bg")
        .. "]Lv.3 ["
        .. (self.level == 3 and "fg" or "light_bg")
        .. "]Effect - "
        .. (
          self.level == 3 and character_effect_names[self.character]
          or character_effect_names_gray[self.character]
        ),
      font = pixul_font,
      alignment = "center",
      height_multiplier = 1.25,
    ...
    {
      text = (
        self.level == 3 and character_effect_descriptions[self.character]()
        or character_effect_descriptions_gray[self.character]()
      ),
      font = pixul_font,
      alignment = "center",
    },
...
function CharacterPart:init(args)
  ...
  self.spring:pull(0.2, 200, 10)
  if self.spawn_effect then
    SpawnEffect({
      group = main.current.effects,
      x = self.x,
      y = self.y,
      color = character_colors[self.character],
    })
  end
...
function CharacterPart:update(dt)
  ...
  if not self.parent:is(CharacterPart) then
    ...
    if self.parent.unit_grabbed and self.parent.unit_grabbed == self then
      local x, y = camera:get_mouse_position()
      local i
      if y >= self.y - 19 and y <= self.y + 19 then
        i = self.i
      elseif y < self.y - 19 then
        i = self.i - 1
      elseif y > self.y + 19 then
        i = self.i + 1
      end
      ...
      self.parent.units[self.i], self.parent.units[i] =
        self.parent.units[i], self.parent.units[self.i]
      self.parent.characters[self.i], self.parent.characters[i] =
        self.parent.characters[i], self.parent.characters[self.i]
  ...
  if self.selected and input.m2.pressed and not self.just_created then
    _G[random:table({ "coins1", "coins2", "coins3" })]:play({
    ...
    if self.reserve then
      ...
      self.parent.party_text:set_text({
        {
          text = "[wavy_mid, fg]party " .. tostring(#self.parent.units) .. "/" .. tostring(
            max_units
          ),
          font = pixul_font,
          alignment = "center",
      ...
      self.parent.parent.units[self.i].reserve[self.level] = self.parent.parent.units[self.i].reserve[self.level]
        - 1
      self:die()
      ...
      system.save_run(
        ...
        self.parent.parent.loop,
        gold,
        self.parent.parent.units,
        self.parent.parent.passives,
        self.parent.parent.shop_level,
        self.parent.parent.shop_xp,
        run_passive_pool,
        locked_state
      )
    end
...
function CharacterPart:draw(y)
  ...
  if self.grabbed then
    ...
    graphics.rectangle(
      ...
      3,
      3,
      self.highlighted and bg[10] or character_colors[self.character]
    )
    graphics.print_centered(
      self.level,
      pixul_font,
      ...
      0,
      0,
      self.highlighted and bg[5] or _G[character_color_strings[self.character]][-5]
    )
  end
  if y then
  ...
  self.info_text:activate({
    {
      text = "["
        ...
        .. self.level
        .. "[fg], tier [yellow]"
        .. character_tiers[self.character]
        .. "[fg] - sells for [yellow]"
        .. self:get_sale_price(),
      font = pixul_font,
      alignment = "center",
...
function CharacterPart:get_sale_price()
  if not character_tiers[self.character] then
    return 0
  end
  local total = 0
  total = total
    + (
      (self.level == 1 and character_tiers[self.character])
      or (self.level == 2 and 2 * character_tiers[self.character])
      or (self.level == 3 and 6 * character_tiers[self.character])
      or 0
    )
  if self.reserve then
    if self.reserve[2] then
      total = total + self.reserve[2] * character_tiers[self.character] * 2
    end
    if self.reserve[1] then
      total = total + self.reserve[1] * character_tiers[self.character]
    end
  end
  return total
end
...
function PassiveCard:update(dt)
  ...
  if
    ((self.selected and input.m1.pressed) or input[tostring(self.card_i)].pressed)
    and self.arena.choosing_passives
  then
...
function ItemCard:init(args)
  ...
  self.shape = Rectangle(self.x, self.y, self.w, self.h)
  self.interact_with_mouse = true
  self.max_xp = (self.level == 0 and 0)
    or (self.level == 1 and 2)
    or (self.level == 2 and 3)
    or (self.level == 3 and 0)
  self.unlevellable = table.any(unlevellable_items, function(v)
    return v == self.passive
  end)
end
...
function ItemCard:update(dt)
    ...
    if gold < 5 then
      ...
      if self.xp >= self.max_xp then
        ...
        if self.level == 2 then
          spawn_mark1:play({ pitch = 1, volume = 0.6 })
        end
        if self.level == 3 then
  ...
  if self.selected and input.m2.pressed then
    ...
    self.parent:gain_gold(
      (self.level == 1 and 10) or (self.level == 2 and 20) or (self.level == 3 and 30)
    )
...
function ItemCard:create_info_text()
  ...
  if self.level == 3 or self.unlevellable then
    ...
    self.info_text:activate({
      {
        text = "[fg]" .. passive_names[self.passive] .. ", [yellow]Lv." .. self.level,
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25,
      },
      {
        text = passive_descriptions_level[self.passive](self.level),
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25,
      },
    ...
    self.info_text:activate({
      {
        text = "[fg]"
          ...
          .. "/"
          .. self.max_xp
          .. "[fg], +1 XP cost: [yellow]5[fg], sells for: [yellow]"
          .. tostring(
            (self.level == 1 and 10) or (self.level == 2 and 20) or (self.level == 3 and 30)
          ),
        font = pixul_font,
...
function ShopCard:init(args)
  ...
  for i, class in ipairs(character_classes[self.unit]) do
    local x = self.x
    if #character_classes[self.unit] == 2 then
      x = self.x - 10
    elseif #character_classes[self.unit] == 3 then
      x = self.x - 20
    end
    table.insert(
      self.class_icons,
...
function ShopCard:refresh()
  self.owned = table.any(self.parent.units, function(v)
    return v.character == self.unit
  end)
  if self.owned then
    self.owned_n = 0
    for _, unit in ipairs(self.parent.units) do
      if unit.character == self.unit then
        self.owned_n = self.owned_n
          + ((unit.level == 1 and 1) or (unit.level == 2 and 3) or (unit.level == 3 and 9))
...
function ShopCard:update(dt)
  ...
  if (self.selected and input.m1.pressed) or input[tostring(self.i)].pressed then
    if self.parent:buy(self.unit, self.i) then
      ...
      for _, ci in ipairs(self.class_icons) do
        ci.spring:pull(0.2, 200, 10)
      end
    end
  end
end
  ...
  self.t:every_immediate(1.4, function()
    if self.selected then
      self.t:tween(
        0.7,
        self,
        { sx = 0.97, sy = 0.97, plus_r = -math.pi / 32 },
        math.linear,
        function()
          self.t:tween(
            0.7,
            self,
            { sx = 1.03, sy = 1.03, plus_r = math.pi / 32 },
            math.linear,
            nil,
            "pulse_1"
          )
...
function ShopCard:die(dont_spawn_effect)
  ...
  for _, class_icon in ipairs(self.class_icons) do
    class_icon:die(dont_spawn_effect)
  end
  if self.info_text then
    self.info_text:deactivate()
    self.info_text.dead = true
...
function CharacterIcon:draw()
  ...
  graphics.print_centered(
    ...
    0,
    0,
    _G[character_color_strings[self.character]][-5]
  )
...
function CharacterIcon:on_mouse_enter()
  ...
  self.info_text:activate({
    ...
    {
      text = character_descriptions[self.character](1),
      font = pixul_font,
      alignment = "center",
      height_multiplier = 2,
    },
...
function TutorialClassIcon:draw()
  ...
  graphics.rectangle(
    ...
    4,
    4,
    self.highlighted and fg[0] or ((n >= i) and class_colors[self.class] or bg[3])
  )
  _G[self.class]:draw(
    self.x,
    self.y,
    ...
    0,
    0,
    self.highlighted and fg[-5] or ((n >= i) and _G[class_color_strings[self.class]][-5] or bg[10])
  )
  ...
  if i == 2 and not k then
    if self.highlighted then
      ...
      graphics.line(
        ...
        self.x - 3,
        self.y + 25,
        (n >= 1) and class_colors[self.class] or bg[10],
        3
      )
      graphics.line(
        self.x - 3,
        ...
        self.x - 3,
        self.y + 32,
        (n >= 2) and class_colors[self.class] or bg[10],
        3
      )
      graphics.line(
        self.x + 4,
        ...
        self.x + 4,
        self.y + 25,
        (n >= 3) and class_colors[self.class] or bg[10],
        3
      )
      graphics.line(
        self.x + 4,
        ...
        self.x + 4,
        self.y + 32,
        (n >= 4) and class_colors[self.class] or bg[10],
        3
      )
    end
  elseif i == 3 then
    if self.highlighted then
      ...
      graphics.line(
        ...
        self.x + 4,
        self.y + 27,
        (n >= 5) and class_colors[self.class] or bg[10],
        3
      )
      graphics.line(
        self.x + 4,
        ...
        self.x + 4,
        self.y + 32,
        (n >= 6) and class_colors[self.class] or bg[10],
        3
      )
    end
  end
...
function TutorialClassIcon:on_mouse_enter()
  ...
  self.info_text:activate({
    {
      text = "["
        .. class_color_strings[self.class]
        .. "]"
        .. self.class:capitalize()
        .. "[fg] - owned: [yellow]"
        .. owned,
      font = pixul_font,
      alignment = "center",
    ...
    {
      text = class_descriptions[self.class]((owned >= j and 2) or (owned >= i and 1) or 0),
      font = pixul_font,
      alignment = "center",
    },
  }, nil, nil, nil, nil, 16, 4, nil, 2)
...
function ClassIcon:draw()
  ...
  if self.parent:is(ShopCard) then
    ...
    if table.any(self.units, function(v)
      return v.character == self.character
    end) then
      next_n = nil
    end
  end
    ...
    if next_n then
      if next_n == 1 then
        graphics.rectangle(
          ...
          nil,
          nil,
          self.flash and class_colors[self.class] or bg[10]
        )
      end
    end
...
function ClassIcon:on_mouse_enter()
  ...
  self.info_text:activate({
    ...
    },
    {
      text = class_descriptions[self.class](
        (k and (owned >= k and 3)) or (owned >= j and 2) or (owned >= i and 1) or 0
      ),
      font = pixul_font,
      alignment = "center",
    },
  ...
  if not self.parent:is(ShopCard) then
    for _, character in ipairs(self.parent.characters) do
      if
        table.any(character_classes[character.character], function(v)
          return v == self.class
        end)
      then
        character:highlight()
...
function ClassIcon:die(dont_spawn_effect)
  ...
  if not dont_spawn_effect then
    SpawnEffect({
      ...
      x = self.x,
      y = self.y + 4,
      color = (n >= i) and class_colors[self.class] or bg[3],
    })
  end
  if self.info_text then
    self.info_text:deactivate()
  ...
  if self.selected and not self.parent:is(ShopCard) then
    for _, character in ipairs(self.parent.characters) do
      if table.any(character.classes, function(v)
        return v == self.class
      end) then
        character:highlight()
      end
