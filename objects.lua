...
function LightningLine:init(args)
  ...
  self:generate()
  self.t:tween(self.duration or 0.1, self, { w = 1 }, math.linear, function()
    self.dead = true
  end)
  ...
  HitCircle({
    ...
    rs = 6,
    color = fg[0],
    duration = self.duration or 0.1,
  })
  for i = 1, 2 do
    HitParticle({ group = main.current.effects, x = self.src.x, y = self.src.y, color = self.color })
  end
  HitCircle({
    group = main.current.effects,
    x = self.dst.x,
...
function WallKnife:init(args)
  ...
  self.dvr = random:table({
    random:float(-8 * math.pi, -4 * math.pi),
    random:float(4 * math.pi, 8 * math.pi),
  })
end
...
function Unit:calculate_stats(first_run)
  ...
  elseif self:is(Seeker) then
    if current_new_game_plus == 0 then
      if self.boss then
        local x = self.level
        local y =
          { 0, 0, 3, 0, 0, 6, 0, 0, 9, 0, 0, 12, 0, 0, 18, 0, 0, 40, 0, 0, 32, 0, 0, 64, 90 }
        local y2 =
          { 0, 0, 24, 0, 0, 28, 0, 0, 32, 0, 0, 36, 0, 0, 44, 0, 0, 64, 0, 0, 48, 0, 0, 80, 100 }
        local k = 1.07
        for i = 26, 50 do
          y[i] = y2[i - 25]
        end
      ...
      if self.boss then
        ...
        self.base_mvspd = math.min(35 + 1.5 * y[x], 35 + 1.5 * y[150])
        if x % 25 == 0 then
          self.base_dmg = (12 + current_new_game_plus * 2)
            + (1.75 + 0.5 * current_new_game_plus) * y[x]
        ...
        self.base_dmg = (4 + current_new_game_plus * 1.15)
          + (2 + current_new_game_plus * 0.83) * y[x]
        self.base_mvspd = math.min(70 + 3 * y[x], 70 + 3 * y[150])
      end
    end
  ...
  elseif self:is(EnemyCritter) or self:is(Critter) then
    local x = self.level
    local y =
      { 0, 1, 3, 3, 4, 6, 5, 6, 9, 7, 8, 12, 10, 11, 15, 12, 13, 18, 16, 17, 21, 17, 20, 24, 25 }
    local k = 1.2
    for i = 26, 5000 do
      local n = i % 25
      if n == 0 then
  ...
  for _, class in ipairs(self.classes) do
    self.class_hp_m = self.class_hp_m * class_stat_multipliers[class].hp
  end
  ...
  for _, class in ipairs(self.classes) do
    self.class_dmg_m = self.class_dmg_m * class_stat_multipliers[class].dmg
  end
  self.dmg = (self.base_dmg + self.class_dmg_a + self.buff_dmg_a)
    * self.class_dmg_m
    * self.buff_dmg_m

  for _, class in ipairs(self.classes) do
    self.class_aspd_m = self.class_aspd_m * class_stat_multipliers[class].aspd
  end
  self.aspd_m = 1 / (self.base_aspd_m * self.class_aspd_m * self.buff_aspd_m)

  for _, class in ipairs(self.classes) do
    self.class_area_dmg_m = self.class_area_dmg_m * class_stat_multipliers[class].area_dmg
  end
  self.area_dmg_m = self.base_area_dmg_m * self.class_area_dmg_m * self.buff_area_dmg_m

  for _, class in ipairs(self.classes) do
    self.class_area_size_m = self.class_area_size_m * class_stat_multipliers[class].area_size
  end
  self.area_size_m = self.base_area_size_m * self.class_area_size_m * self.buff_area_size_m

  for _, class in ipairs(self.classes) do
    self.class_def_m = self.class_def_m * class_stat_multipliers[class].def
  end
  self.def = (self.base_def + self.class_def_a + self.buff_def_a)
    * self.class_def_m
    * self.buff_def_m

  for _, class in ipairs(self.classes) do
    self.class_mvspd_m = self.class_mvspd_m * class_stat_multipliers[class].mvspd
  end
  self.max_v = (self.base_mvspd + self.class_mvspd_a + self.buff_mvspd_a)
    * self.class_mvspd_m
    * self.buff_mvspd_m
  self.v = (self.base_mvspd + self.class_mvspd_a + self.buff_mvspd_a)
    * self.class_mvspd_m
    * self.buff_mvspd_m
end
  ...
  graphics.line(
    ...
    p.hfx.hit.f and fg[0]
      or (
        ...
        or (
          table.any(main.current.enemies, function(v)
            return p:is(v)
          end) and red[0]
        )
      ),
