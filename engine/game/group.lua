...
function Group:update(dt)
  ...
  for _, object in ipairs(self.objects) do
    ...
    if not self.cells[cx] then
      self.cells[cx] = {}
    end
    if not self.cells[cx][cy] then
      self.cells[cx][cy] = {}
    end
...
function Group:draw_all_except(classes, scroll_factor_x, scroll_factor_y)
  ...
  for _, object in ipairs(self.objects) do
    if
      not table.any(classes, function(v)
        return object:is(v)
      end) and not object.hidden
    then
      object:draw()
    end
  end
...
function Group:get_objects_in_shape(shape, object_types, exclude_list)
  ...
  local cx1, cy1 =
    math.floor((shape.x - shape.w) / self.cell_size),
        ...
        if cell_objects then
          for _, object in ipairs(cell_objects) do
            if object_types then
              if
                not table.any(exclude_list, function(v)
...
function Group:set_as_physics_world(meter, xg, yg, tags)
  ...
  self.world:setCallbacks(function(fa, fb, c)
    local oa, ob = self:get_object_by_id(fa:getUserData()), self:get_object_by_id(fb:getUserData())
    if fa:isSensor() or fb:isSensor() then
      if fa:isSensor() then
        if oa.on_trigger_enter then
          oa:on_trigger_enter(ob, c)
        end
      end
      if fb:isSensor() then
        if ob.on_trigger_enter then
          ob:on_trigger_enter(oa, c)
        end
      end
    else
      if oa.on_collision_enter then
        oa:on_collision_enter(ob, c)
      end
      if ob.on_collision_enter then
        ob:on_collision_enter(oa, c)
      end
    end
  end, function(fa, fb, c)
    local oa, ob = self:get_object_by_id(fa:getUserData()), self:get_object_by_id(fb:getUserData())
    if fa:isSensor() or fb:isSensor() then
      if fa:isSensor() then
        if oa.on_trigger_exit then
          oa:on_trigger_exit(ob, c)
        end
      end
      if fb:isSensor() then
        if ob.on_trigger_exit then
          ob:on_trigger_exit(oa, c)
        end
      end
    else
      if oa.on_collision_exit then
        oa:on_collision_exit(ob, c)
      end
      if ob.on_collision_exit then
        ob:on_collision_exit(oa, c)
      end
    end
  end)
  return self
