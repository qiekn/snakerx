...
function Physics:set_as_line(x1, y1, x2, y2, body_type, tag)
  ...
  local shape =
    love.physics.newEdgeShape(self.shape.x1, self.shape.y1, self.shape.x2, self.shape.y2)
  self.fixture = love.physics.newFixture(self.body, shape)
  self.fixture:setUserData(self.id)
  self.fixture:setCategory(self.group.collision_tags[tag].category)
  self.fixture:setMask(unpack(self.group.collision_tags[tag].masks))
...
function Physics:connect(other, direction)
  ...
  self.joints[direction] = love.physics.newRevoluteJoint(
    self.body,
    other.body,
    self.x + 0.5 * d.x * self.shape.w,
    self.y + 0.5 * d.y * self.shape.h
  )
  return self
end
...
function Physics:get_random_object_in_shape(shape, object_types, exclude_list)
  ...
  if random_object then
    while
      table.any(exclude_list, function(v)
...
function Physics:rotate_towards_object(object, lerp_value)
  self:set_angle(
    math.lerp_angle(lerp_value, self:get_angle(), self:angle_to_point(object.x, object.y))
  )
  return self
end
...
function Physics:rotate_towards_velocity(lerp_value)
  local vx, vy = self:get_velocity()
  self:set_angle(
    math.lerp_angle(lerp_value, self:get_angle(), self:angle_to_point(self.x + vx, self.y + vy))
  )
  return self
end
...
function Physics:accelerate_towards_point(x, y, max_speed, deceleration, turn_coefficient)
  ...
  if d > 0 then
    ...
    local vx, vy = self:get_velocity()
    self:apply_force(
      (current_vx - vx) * (turn_coefficient or 1),
      (current_vy - vy) * (turn_coefficient or 1)
    )
  end
  return self
end
...
function Physics:accelerate_towards_object(object, max_speed, deceleration, turn_coefficient)
  return self:accelerate_towards_point(
    object.x,
    object.y,
    max_speed,
    deceleration,
