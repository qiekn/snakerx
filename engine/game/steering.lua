...
function Physics:calculate_steering_force(dt)
  ...
  if self.seeking then
    self.steering_force:add(self.seek_f)
  end
  if self.fleeing then
    self.steering_force:add(self.flee_f)
  end
  if self.pursuing then
    self.steering_force:add(self.pursuit_f)
  end
  if self.evading then
    self.steering_force:add(self.evade_f)
  end
  if self.wandering then
    self.steering_force:add(self.wander_f)
  end
  if self.path_following then
    self.steering_force:add(self.path_follow_f)
  end
  if self.separating then
    self.steering_force:add(self.separation_f)
  end
  if self.aligning then
    self.steering_force:add(self.alignment_f)
  end
  if self.cohesing then
    self.steering_force:add(self.cohesion_f)
  end
  self.seeking = false
  self.fleeing = false
  self.pursuing = false
...
function Physics:calculate_applied_force(dt)
  ...
  if self.applying_force then
    self.applied_force:add(self.apply_force_f)
  end
...
function Physics:calculate_applied_impulse(dt)
  ...
  if self.applying_impulse then
    self.applied_impulse:add(self.apply_impulse_f)
  end
...
function Physics:seek_point(x, y, deceleration, weight)
  ...
  if d > 0 then
    ...
    local vx, vy = self:get_velocity()
    self.seek_f:set(
      (dvx - vx) * self.turn_multiplier * (weight or 1),
      (dvy - vy) * self.turn_multiplier * (weight or 1)
    )
  else
    self.seek_f:set(0, 0)
  end
...
function Physics:wander(rs, distance, jitter, weight)
  ...
  local target_world =
    steering.point_to_world_space(target_local, self.heading, self.side, Vector(self.x, self.y))
  self.wander_f:set(
    (target_world.x - self.x) * (weight or 1),
    (target_world.y - self.y) * (weight or 1)
  )
end
...
function C2DMatrix:multiply(other)
  ...
  self._11 = mat_temp._11
  self._12 = mat_temp._12
  self._13 = mat_temp._13
  self._21 = mat_temp._21
  self._22 = mat_temp._22
  self._23 = mat_temp._23
  self._31 = mat_temp._31
  self._32 = mat_temp._32
  self._33 = mat_temp._33
end
...
function steering.point_to_local_space(point, heading, side, position)
  ...
  mat_transform._11 = heading.x
  mat_transform._12 = side.x
  mat_transform._21 = heading.y
  mat_transform._22 = side.y
  mat_transform._31 = tx
  mat_transform._32 = ty
  mat_transform:transform_vector(trans_point)
  return trans_point
end
