...
function Flashes:add(name, default_duration)
  ...
  then
    error(
      "Invalid name to be added to the Flashes object. 'add', 'flash', 'init', 'names', 'parent', 'pull', 'trigger', 'update' and 'use' are reserved names, choose another."
    )
  end
  self[name] = {
    f = false,
    default_duration = default_duration or 0.15,
    flash = function(_, duration)
      self[name].f = true
      self.trigger:after(duration or self[name].default_duration, function()
        self[name].f = false
      end, name)
    end,
  }
