...
function Shake:init(amplitude, duration, frequency)
  ...
  self.samples = {}
  for i = 1, (duration / 1000) * frequency do
...
function Shake:get_amplitude(t)
  ...
  return self.amplitude
    * (self:get_noise(s0) + (s - s0) * (self:get_noise(s1) - self:get_noise(s0)))
    * k
end
...
function Camera:update(dt)
  ...
  self.mouse_dt.x, self.mouse_dt.y =
    self.mouse.x - self.last_mouse.x, self.mouse.y - self.last_mouse.y
  ...
  if self.bound then
    self.x =
      math.min(math.max(self.x, self.bounds_min.x + self.w / 2), self.bounds_max.x - self.w / 2)
    self.y =
      math.min(math.max(self.y, self.bounds_min.y + self.h / 2), self.bounds_max.y - self.h / 2)
  end
  ...
  if self.follow_style == "screen_by_screen" then
    if self.bound then
      if self.x > self.bounds_min.x + self.w / 2 and target_x < 0 then
        self.scroll.x = math.snap_center(self.scroll.x - self.w / self.sx, self.w / self.sx)
      end
      if self.x < self.bounds_max.x - self.w / 2 and target_x >= self.w then
        self.scroll.x = math.snap_center(self.scroll.x + self.w / self.sx, self.w / self.sx)
      end
      if self.y > self.bounds_min.y + self.h / 2 and target_y < 0 then
        self.scroll.y = math.snap_center(self.scroll.y - self.h / self.sy, self.h / self.sy)
      end
      if self.y < self.bounds_max.y - self.h / 2 and target_y >= self.h then
        self.scroll.y = math.snap_center(self.scroll.y + self.h / self.sy, self.h / self.sy)
      end
    else
      if target_x < 0 then
