...
function graphics.update(dt)
  for i = #self.debug_queries, 1, -1 do
    ...
    if query.frames <= 0 then
      table.remove(self.debug_queries, i)
    end
  end
end
...
function graphics.print_centered(text, font, x, y, r, sx, sy, ox, oy, color)
  ...
  love.graphics.print(
    ...
    sx or 1,
    sy or 1,
    (ox or 0) + font:get_text_width(text) / 2,
    (oy or 0) + font.h / 2
  )
  if color then
    love.graphics.setColor(_r, g, b, a)
...
function graphics.dashed_rounded_line(x1, y1, x2, y2, dash_size, gap_size, color, line_width)
  ...
  for i = 0, nm do
    ...
    graphics.shape(
      ...
      i * st,
      0,
      math.length((i * st + dash_size) - (i * st), 0 - 0),
      line_width / 2,
      line_width / 4,
      line_width / 4
    )
  ...
  graphics.shape(
    ...
    nm * st,
    0,
    math.length((nm * st + dash_size) - (nm * st), 0 - 0),
    line_width / 2,
    line_width / 4,
    line_width / 4
  )
...
function graphics.draw_with_mask(action, mask_action, invert_mask)
  ...
  if not invert_mask then
    graphics.set_stencil_test("greater", 0)
  else
    graphics.set_stencil_test("notequal", 1)
  end
  action()
...
function graphics.draw_intersection(action1, action2)
  graphics.stencil(function()
    love.graphics.setShader(stencil_mask_shader)
    action1()
    love.graphics.setShader()
  end, "replace", 1)
  graphics.set_stencil_test("greater", 0)
...
function graphics.generate_rtfx_animation(w, h)
  ...
  for _, file in ipairs(files) do
    table.insert(frames, love.image.newImageData(file))
  end
