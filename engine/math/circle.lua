...
function Circle:init(x, y, rs)
  ...
  self.x1, self.y1, self.x2, self.y2 =
    self.x - self.rs, self.y - self.rs, self.x + self.rs, self.y + self.rs
end
...
function Circle:is_colliding_with_circle(circle)
  return mlib.circle.isCircleCompletelyInside(
    self.x,
    self.y,
    self.rs,
    circle.x,
    circle.y,
    circle.rs
  ) or mlib.circle.isCircleCompletelyInside(
    circle.x,
    circle.y,
    circle.rs,
    self.x,
    self.y,
    self.rs
  ) or mlib.circle.getCircleIntersection(self.x, self.y, self.rs, circle.x, circle.y, circle.rs)
end
...
function Circle:is_colliding_with_polygon(polygon)
  return mlib.polygon.isCircleCompletelyInside(self.x, self.y, self.rs, polygon.vertices)
    or mlib.circle.isPolygonCompletelyInside(self.x, self.y, self.rs, polygon.vertices)
    or mlib.polygon.getCircleIntersection(self.x, self.y, self.rs, polygon.vertices)
end
