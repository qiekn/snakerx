...
function Polygon:inflate(s)
  self.vertices = path_to_polygon(
    clipper
      .ClipperOffset()
      :offsetPath(polygon_to_path(self.vertices), s, "miter", "closedPolygon")
      :get(1)
  )
end
...
function Polygon:scale(sx, sy, ox, oy)
  for i = 1, #self.vertices, 2 do
    self.vertices[i], self.vertices[i + 1] = math.scale_point(
      self.vertices[i],
      self.vertices[i + 1],
      sx or 1,
      sy or sx or 1,
...
function Polygon:rotate(r, ox, oy)
  for i = 1, #self.vertices, 2 do
    self.vertices[i], self.vertices[i + 1] = math.rotate_point(
      self.vertices[i],
      self.vertices[i + 1],
      r,
      ox or self.cx or 0,
...
function Polygon:is_colliding_with_polygon(polygon)
  return mlib.polygon.isPolygonInside(self.vertices, polygon.vertices)
    or mlib.polygon.isPolygonInside(polygon.vertices, self.vertices)
end
